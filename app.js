...
function App() {
  ...
  const generatePrettyReport = (partyList, preview = false) => {
    ...
    const repAccounts = scoped ? data.accounts.map(acc => {
      const ts = trComputed.filter(t => t.accountId === acc.id && sel.has(t.fromParty));
      return {
        ...acc,
        count: ts.length,
        usd: ts.reduce((s, t) => s + t.usd, 0)
    ...
    const partySections = partyList.map(p => {
      ...
      const ledger = st.rows.length ? opening + st.rows.map(r => `<tr><td>${fmtD(r.date)}</td><td>${r.kind === "in" ? "Received" : "Payment Out"}</td><td>${r.desc} <span class="muted">(${r.currency} ${money(r.amount)})${r.comment ? " · " + r.comment : ""}</span></td><td class="r">${r.kind === "in" ? usd(r.usd) : ""}</td><td class="r">${r.kind === "out" ? usd(r.usd) : ""}</td><td class="r b">${usd(r.balance)}</td></tr>`).join("") : `<tr><td colspan="6" class="empty">No movements yet.</td></tr>`;
  ...
  const exportExcel = (partyList = []) => {
    ...
    addStyledSheet("Agreements", fAg.map(a => ({
      Ref: a.ref,
      Title: a.title,
      Client: a.party,
      Date: a.date,
    ...
    addStyledSheet("Invoices", fAg.flatMap(a => (a.invoices || []).map(iv => ({
    ...
    addStyledSheet("Receipts", fAg.flatMap(a => (a.receipts || []).map(r => ({
      Date: r.date,
      Agreement: a.title,
      Ref: a.ref,
      Client: a.party,
    ...
    addStyledSheet("Disbursements", fDisb.map(d => ({
      Date: d.date,
      Party: d.party,
    ...
    addStyledSheet("Disb Payments", fDisb.flatMap(d => (d.payments || []).map(p => ({
      Date: p.date,
      Party: d.party,
      "Source Agreement": d.agreementTitle,
      Currency: d.currency,
    ...
    addStyledSheet("Transfers", fTr.map(t => ({
      Date: t.date,
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    onPreview: list => {
      setPreviewHtml(generatePrettyReport(list, true));
    },
    onGenerate: (which, list) => {
      if (which === "pretty" || which === "both") generatePrettyReport(list);
...
function ReportModal({
  ...
  return /*#__PURE__*/React.createElement(Modal, {
    ...
    key: p,
    onClick: () => toggle(p),
    className: `px-3 py-1 rounded-full text-xs border transition-colors ${pick.includes(p) ? "bg-slate-900 text-white border-slate-900" : "bg-white text-slate-600 border-slate-300 hover:border-slate-500"}`
  }, p))), /*#__PURE__*/React.createElement("p", {
    className: "text-xs text-slate-400 mt-1"
  }, pick.length, " selected \xB7 the report (agreements, accounts and statements) covers only the selected parties. Select none for an all-parties report.")), /*#__PURE__*/React.createElement("div", {
    className: "flex gap-2"
  }, /*#__PURE__*/React.createElement("button", {
