...
class ErrorBoundary extends Component {
  ...
  render() {
    if (this.state.error) return /*#__PURE__*/React.createElement("div", {
      className: "p-6 m-4 bg-rose-50 border border-rose-200 rounded-xl text-sm text-rose-700"
    }, /*#__PURE__*/React.createElement("b", null, "Something went wrong on this page."), /*#__PURE__*/React.createElement("div", {
      className: "mt-1 text-xs font-mono whitespace-pre-wrap"
    }, String(this.state.error && (this.state.error.message || this.state.error) || "Unknown error")), /*#__PURE__*/React.createElement("button", {
      onClick: () => this.setState({
        error: null
      }),
      className: "mt-3 text-xs px-3 py-1.5 rounded-lg bg-rose-600 text-white"
    }, "Try again"));
...
const DEFAULT_PARTIES = ["AF", "CB***", "CB", "RS", "PCA", "CP", "Cash (Others)", "O. Dev"].map((name, i) => ({
  id: "dp" + i,
  name,
  type: "disbursement"
}));
...
const orderName = (a, b) => String(a || "").localeCompare(String(b || ""), undefined, {
  numeric: true,
  sensitivity: "base"
});
const orderPartyName = (a, b) => {
  const pin = n => n === "O. Dev" ? 2 : /^cash/i.test(n) ? 1 : 0;
  return pin(a) - pin(b) || orderName(a, b);
};
...
const stGet = async (k, shared = SHARED) => {
  try {
...
const stSet = async (k, v, shared = SHARED) => {
  try {
    await window.storage.set(k, v, shared);
  } catch (e) {
...
const sbGet = async () => {
  if (!SB) return null;
  try {
    const r = await fetch(`${SB.url}/rest/v1/${SB.table}?id=eq.${encodeURIComponent(SB.row)}&select=data,updated_at`, {
      headers: sbHeaders(),
      cache: "no-store"
    });
...
const loadSyncConfig = async () => {
  try {
    const r = await fetch("notes-sync.json", {
      cache: "no-store"
    });
    if (r.ok) {
      const c = await r.json();
...
const teamGet = async () => {
  ...
  try {
    ...
    return {
      notes: Array.isArray(j) ? j : Array.isArray(j.notes) ? j.notes : [],
      users: Array.isArray(j && j.users) ? j.users : []
    };
  } catch (e) {
    console.warn("Shared read failed:", e.message);
    return null;
  }
};
const teamPut = async doc => {
  ...
  try {
    ...
    console.warn("Shared write failed:", e.message);
    return false;
  }
};
const uid = () => Date.now().toString(36) + Math.random().toString(36).slice(2, 7);
const fmt = n => isNaN(n) ? "0.00" : Number(n).toLocaleString("en-US", {
...
const Amt = ({
  ...
  return /*#__PURE__*/React.createElement("span", {
    className: `flex items-baseline justify-between gap-3 w-full tabular-nums ${className}`
  }, /*#__PURE__*/React.createElement("span", {
    className: "opacity-50 font-normal"
  }, sym), /*#__PURE__*/React.createElement("span", null, n < -0.005 ? `(${fmt(-n)})` : fmt(n)));
};
...
const AmtTile = ({
  ...
  return /*#__PURE__*/React.createElement("span", {
    ...
    className: "opacity-40 font-normal text-[0.7em]"
  }, sym), /*#__PURE__*/React.createElement("span", null, n < -0.005 ? `(${fmt(-n)})` : fmt(n)));
};
const AmtG = ({
  v,
  ...
  const n = Number(v || 0);
  return /*#__PURE__*/React.createElement("span", {
    className: `inline-flex items-baseline justify-end gap-1.5 tabular-nums ${className}`
  }, /*#__PURE__*/React.createElement("span", {
    className: "inline-block text-right",
    style: {
      minWidth: "6.5rem"
    }
  }, n < -0.005 ? `(${fmt(-n)})` : fmt(n)), /*#__PURE__*/React.createElement("span", {
...
const PartyName = ({
  ...
  className: className
}, name, name === "O. Dev" && /*#__PURE__*/React.createElement("span", {
  className: "block text-[10px] italic text-slate-400 font-normal leading-tight"
}, "(NOT Service Fee)"));
...
const pd = s => {
  const m = /(\d{1,2})-(\w{3})-(\d{2})/.exec(s || "");
  return m ? `20${m[3]}-${MON[m[2]]}-${String(m[1]).padStart(2, "0")}` : "";
};
...
const backfillInvoices = d => {
  ...
  const agreements = d.agreements.map(a => {
    ...
    groups.forEach((g, i) => g.rows.forEach(r => {
      link[r.id] = invoices[i].id;
    }));
    touched = true;
    return {
      ...a,
      invoices,
      receipts: receipts.map(r => r.invoiceId ? r : {
        ...r,
        invoiceId: link[r.id] || ""
      })
    };
...
const autoLinkReceipts = d => {
  ...
  const agreements = d.agreements.map(a => {
    ...
    const bal = {};
    invs.forEach(iv => {
      bal[iv.id] = Number(iv.amount || 0);
    });
    rcpts.forEach(r => {
      if (r.invoiceId && ids.has(r.invoiceId)) bal[r.invoiceId] -= Number(r.amount || 0);
    });
...
const REGISTER = [{
  ...
  total: 300000,
  status: "Closed",
  receipts: [["28-Jun-24", "Inv #1 (000069)", 300000]],
  alloc: {
    AF: 300000
  }
}, {
  ...
  total: 1250000,
  status: "Closed",
  receipts: [["1-Jun-24", "Inv #1 (000067)", 750000], ["15-Jun-24", "Inv #2 (000068)", 500000]],
  alloc: {
    AF: 1250000
  }
}, {
  ...
  total: 813848.59,
  status: "Ongoing",
  receipts: [["28-Oct-24", "Inv #1 (INV-000074)", 34361.25, 38286], ["28-Dec-24", "Inv #2 (INV-000081)", 110250, 119325.78], ["5-Feb-25", "Inv #3 (INV-000082)", 98000, 106067.36], ["16-Jun-25", "Inv #4 (00000093)", 31556, 34153.69], ["14-Jun-25", "Inv #5 (0000094)", 73631, 79692.57], ["9-Jul-25", "Inv #6 (00000097)", 105188, 113846.54], ["10-Oct-25", "Inv #7 (000000100)", 32465.24, 32960.80], ["14-Nov-25", "Inv #8 (000000101)", 75752.20, 89979.77], ["15-Dec-25", "Inv #9 (000000102)", 108217.43, 128542.53], ["31-Jan-26", "Inv #10 (000000107)", 25675.17, 29625.29], ["31-Mar-26", "Inv #10 (000000108)", 72746.29, 78060.41]],
  alloc: {
    "CB***": 850540.89
  }
}, {
  ...
  total: 200000,
  status: "Closed",
  receipts: [["1-Dec-24", "Inv #1 (INV 000184)", 200000]],
  alloc: {
    CB: 20000,
    RS: 180000
  }
  ...
  total: 99885.05,
  status: "Closed",
  receipts: [["20-Mar-25", "Inv #1 (INV-000089)", 99885]],
  alloc: {
    CB: 10000,
    RS: 89885
  }
  ...
  total: 770000,
  status: "Ongoing",
  receipts: [["1-Oct-25", "Invst#3 (000099)", 179990]],
  alloc: {
    AF: 49000,
    "CB***": 80000,
    RS: 79990
  ...
  total: 576071.23,
  status: "Ongoing",
  receipts: [["3-Nov-25", "Invst #4 (0000XX)", 300000], ["8-Feb-26", "Invst #5 (0000XX)", 276071.23]],
  alloc: {
    AF: 576071.23
  }
}, {
  ref: "9.a",
  title: "DXB _Oak Adjusment",
  cur: "USD",
  total: 526421.23,
  status: "Ongoing",
  receipts: [["31-Jul-25", "Invst#3 (0000XX)", 387671.23], ["3-Apr-26", "Invst #5 (0000XX)", 138750]],
  alloc: {
    AF: 526421.23,
    "Cash (Others)": 151441.14
  }
}, {
  ref: "10",
  title: "Okut 1",
  ...
  total: 653882.30,
  status: "Ongoing",
  note: "Received amounts not logged in USD column on source",
  receipts: [],
  alloc: {
    AF: 309720.57,
    PCA: 192720.57,
    CP: 117000,
    "Cash (Others)": 75720.57
  }
}, {
  ref: "11",
  title: "XYZ (S) 2",
  ...
  total: 1065000,
  status: "Closed",
  receipts: [["1-Nov-25", "Inv #1 (000104)", 395000], ["1-Dec-25", "Inv #2 (000105)", 245000], ["1-Jan-26", "Inv #3 (000106)", 425000]],
  alloc: {
    AF: 1065000
  }
}, {
  ref: "12",
  title: "London (1079) / Conc.",
  cur: "EUR",
  total: 863712,
  status: "Ongoing",
  note: "Invoices not yet received",
  receipts: [],
  alloc: {
    AF: 87287.16
  }
  ...
  total: 270800,
  status: "Closed",
  receipts: [["1-Dec-25", "Inv #1 (66/65/65/63)", 270800, 293092.26]],
  alloc: {
    AF: 146546.13,
    CP: 146546.13
  }
  ...
  ref: "14",
  title: "Cedilha 2026 (400)",
  cur: "EUR",
  total: 400000,
  status: "Ongoing",
  receipts: [["15-Feb-26", "Inv #1 (000111)", 169535.16], ["15-Mar-26", "Inv #2", 58052.45], ["15-Apr-26", "Inv #3", 53791.30]],
  alloc: {
    "Cash (Others)": 448922.05
  }
}, {
  ref: "15",
  title: "Angola _K (1M)",
  ...
  ref: "16",
  title: "Trans Africa (VRD)",
  cur: "EUR",
  total: 1000000,
  status: "Ongoing",
  note: "Allocations to AF; USD receipts not logged on source",
  receipts: [],
  alloc: {
    AF: 765484.26
  }
  ...
  total: 319320,
  status: "Ongoing",
  receipts: [["20-Apr-26", "Inv #1 (000113)", 319320]],
  alloc: {
    "CB***": 319320
  }
}, {
  ...
  total: 1000000,
  status: "Closed",
  receipts: [["5-Feb-26", "Inv #1 (000104)", 325000], ["5-Mar-26", "Inv #2 (000105)", 245000], ["5-Apr-26", "Inv #3 (000106)", 430000]],
  alloc: {
    AF: 1000000
  }
}, {
...
function buildSeed() {
  ...
  REGISTER.forEach(a => {
    ...
    agreements.push({
      ...
      totalValue: a.total,
      status: a.status,
      paymentStatus: a.status === "Closed" ? "Paid" : hasReceipts ? "Ongoing" : "Pending",
      comment: a.note || "",
      receipts
    });
    Object.entries(a.alloc || {}).forEach(([party, amt]) => {
      ...
      disbursements.push({
        ...
        payments: paid ? [{
          ...
          amount: amt,
          rate: 1,
          notes: "Allocated from register"
        }] : []
      });
    });
  });
  const accounts = ["RE Acquisitions", "Undeposited Funds", "Aegis Account", "Soba CBD (Q4 2025)", "Oak - AC", "CP Undeposited", "AEGIS - DA", "CP-AE-CBD", "CP (Emaar)", "DA"].map(n => ({
    id: uid(),
    name: n,
    currency: "USD",
    comment: ""
...
function App() {
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!raw) {
        raw = await stGet("fintrack-auth-v2", false);
        if (raw) await stSet("fintrack-auth-v2", raw);
      }
      if (raw) {
        try {
          const isNamedSuper = id => (id || "").toLowerCase() === SUPER_ADMIN_ID.toLowerCase();
          let arr = JSON.parse(raw).map(u => ({
            ...u,
            role: isNamedSuper(u.userId) || u.role === SUPER ? SUPER : "admin",
            registered: u.registered !== undefined ? u.registered : !!u.hash
          }));
          if (arr.some(u => isNamedSuper(u.userId))) arr = arr.map(u => ({
            ...u,
            role: isNamedSuper(u.userId) ? SUPER : "admin"
          }));else if (arr.length && !arr.some(u => u.role === SUPER)) arr[0] = {
            ...arr[0],
            role: SUPER
          };
          setUsers(arr);
  ...
  const startSession = async userId => {
    ...
    try {
      await window.storage.set("fintrack-session-v1", JSON.stringify({
        expires,
        userId
      }));
    } catch (e) {}
  ...
  const persistUsers = async arr => {
    ...
    pushTeam({
      users: arr
    }); // share accounts so the same login works on every device
  };
  const addUser = async (userId, password, question, answer) => {
    ...
    if (users.some(u => u.userId.toLowerCase() === userId.toLowerCase())) return {
      ok: false,
  ...
  const inviteUser = async (userId, name, password, question, answer) => {
    ...
    if (!isSuperAdmin) return {
      ok: false,
      error: "Only the Super Admin can create user IDs."
    };
    if (!userId) return {
      ok: false,
      error: "Choose a user ID."
  ...
  const saveAppUrl = async u => {
    if (!isSuperAdmin) {
      setNotice("Only the Super Admin can set the app address.");
      return;
    }
    u = (u || "").trim();
    setAppUrl(u);
  ...
  const registerUser = async (userId, password, question, answer) => {
    ...
    if (idx < 0) return {
      ok: false,
      error: "No such user ID. Only user IDs created by the Super Admin can register."
    };
    if (users[idx].registered) return {
      ok: false,
      error: "That user ID is already registered — sign in instead."
    };
    const h = await hash(password);
    const ah = await hash((answer || "").toLowerCase().trim());
    const next = [...users];
  ...
  const renameUser = async (userId, name) => {
    ...
    await persistUsers(users.map(u => u.userId === userId ? {
      ...u,
      name: (name || "").trim()
    } : u));
    return true;
  ...
  const tryLogin = async (userId, password) => {
    ...
    if (!u) return {
      ok: false,
      error: "No account with that user ID. Ask the Super Admin to create one."
    };
    if (!u.registered || !u.hash) return {
      ok: false,
      error: "This user ID has not been registered yet — use Register to set a password."
    };
    const h = await hash(password);
    if (h !== u.hash) return {
      ok: false,
  ...
  const resetPassword = async (userId, answer, newPassword) => {
    ...
    const u = users[idx];
    if (!u.registered || !u.answerHash) return {
      ok: false,
      error: "This user ID has not been registered yet — use Register instead."
    };
    const ah = await hash((answer || "").toLowerCase().trim());
    if (ah !== u.answerHash) return {
      ok: false,
  ...
  const logout = async () => {
    try {
      await window.storage.delete("fintrack-session-v1");
    } catch (e) {}
    setAuthed(false);
    setCurrentUser(null);
  };
  ...
  const ask = (message, onConfirm) => setConfirmState({
    message,
    onConfirm
  });
  ...
  useEffect(() => {
    (async () => {
      const r = await stGet("fintrack-release-v1");
      if (r) {
        try {
          setRelease(JSON.parse(r));
        } catch (e) {}
      }
  ...
  const openOrSave = (html, filename) => {
    ...
    if (saveFile(html, filename)) setNotice(`New tabs are blocked here, so "${filename}" was downloaded instead — open it to view or print.`);else setNotice("Could not open or download the file. Open the app in its own browser window and try again.");
  };
  const printView = () => {
    if (!inFrame()) {
      window.print();
    ...
    const html = generatePrettyReport(Object.keys(partyBalances), true).replace("<body>", '<body onload="setTimeout(function(){try{window.print()}catch(e){}},600)">');
  ...
  const pushTeam = async over => {
    ...
    const doc = {
      notes: over && over.notes || dataRef.current.notes || [],
      users: over && over.users || usersRef.current || []
    };
    teamSyncRef.current.json = JSON.stringify(doc);
    await teamPut(doc);
  };
  const applyData = async (obj, {
    push
  } = {}) => {
    const fixed = normalizeInvoices({
      ...empty,
      ...obj
    });
  ...
  useEffect(() => {
    (async () => {
      ...
      if (!raw) {
        raw = await stGet("fintrack-data-v3", false);
        if (raw) await stSet("fintrack-data-v3", raw);
      }
      if (raw) {
        try {
          await applyData(JSON.parse(raw), {
            push: true
          });
        } catch (e) {}
      } else {
        await applyData(buildSeed(), {
          push: true
        });
      }
      setLoaded(true);
  ...
  useEffect(() => {
    ...
    const apply = async (remote, initial) => {
      ...
      teamSyncRef.current.json = JSON.stringify({
        notes,
        users: usrs
      });
      if (JSON.stringify(notes) !== JSON.stringify(localNotes)) {
        ...
        setData(nd);
        await stSet("fintrack-data-v3", JSON.stringify(nd));
      }
      if (JSON.stringify(usrs) !== JSON.stringify(localUsers)) {
        setUsers(usrs);
        await stSet("fintrack-auth-v2", JSON.stringify(usrs));
      }
      if (initial && (notes.length !== (remote.notes || []).length || usrs.length !== (remote.users || []).length)) await teamPut({
        notes,
        users: usrs
      });
    };
  ...
  const save = async next => {
    if (readOnly) {
      setNotice("Read-only \u2014 data entry happens on the published app only.");
      return;
    }
    setData(next);
    ...
    await stSet("fintrack-data-v3", js);
    if (SB) {
      const ok = await sbSet(next);
      if (ok === null) setNotice("Saved on this device, but the shared copy didn't update (offline?). It will re-sync on your next change.");
    }
  };
  const publishRelease = async (version, includeData) => {
    if (!isSuperAdmin) {
      setNotice("Only the Super Admin can publish a release.");
      return;
    }
  ...
  const setEntryLock = async locked => {
    if (!isSuperAdmin) {
      setNotice("Only the Super Admin can change this.");
      return;
    }
    const rel = {
      ...(release || {
      ...
      host: locked ? pagePrint() : release && release.host || "",
      href: locked ? refPrint() || hrefPrint() : release && release.href || "",
      locked
    };
    await stSet("fintrack-release-v1", JSON.stringify(rel));
    setRelease(rel);
  ...
  const agComputed = data.agreements.map(a => {
    ...
    return {
      ...
      invoicedUSD,
      hasInvoices: hasInv,
      outstandingUSD: invoicedUSD - receivedUSD,
      disbursedUSD,
      allocatedUSD,
      undisbursedUSD: receivedUSD - disbursedUSD
    };
  });
  ...
  const trComputed = data.transfers.map(t => {
    ...
    return {
      ...
      accountCurrency: acc ? acc.currency : "",
      usd: Number(t.amount || 0) * Number(t.rate || 0),
      payType: t.payType || "Full"
    };
  });
  const partyBalances = {};
  disbComputed.forEach(dd => {
    if (!dd.party) return;
    partyBalances[dd.party] = partyBalances[dd.party] || {
      inUSD: 0,
      outUSD: 0
    };
    partyBalances[dd.party].inUSD += dd.paidUSD;
  });
  trComputed.forEach(t => {
    if (!t.fromParty) return;
    partyBalances[t.fromParty] = partyBalances[t.fromParty] || {
      inUSD: 0,
      outUSD: 0
    };
    partyBalances[t.fromParty].outUSD += t.usd;
  });
  const accountTotals = [...data.accounts].sort((a, b) => orderName(a.name, b.name)).map(acc => {
    ...
    return {
      ...acc,
      count: ts.length,
      original: ts.filter(t => t.currency === acc.currency).reduce((s, t) => s + Number(t.amount), 0),
      usd: ts.reduce((s, t) => s + t.usd, 0)
    };
  });
  const orderParty = orderPartyName;
  const disbursementPartyNames = [...new Set([...data.parties.filter(p => p.type === "disbursement" || p.type === "both").map(p => p.name), ...disbComputed.map(d => d.party), ...trComputed.map(t => t.fromParty)])].filter(Boolean).sort(orderParty);
  const allPartyNames = disbursementPartyNames;
  const years = [...new Set([...agComputed.flatMap(a => (a.receipts || []).map(r => yearOf(r.date))), ...disbComputed.flatMap(d => [yearOf(d.date), ...(d.payments || []).map(p => yearOf(p.date))]), ...trComputed.map(t => yearOf(t.date))])].filter(Boolean).sort().reverse();
  const F = filters;
  ...
  const updateReceipt = (agId, recId, patch) => {
    ...
    const ag = data.agreements.find(x => x.id === agId);
    if (ag) upsert("agreements", {
      ...ag,
      receipts: (ag.receipts || []).map(r => r.id === recId ? {
        ...r,
        ...patch
      } : r)
    });
  ...
  const deleteInvoice = (agId, invId) => {
    ...
    upsert("agreements", {
      ...
      receipts: (ag.receipts || []).map(r => r.invoiceId === invId ? {
        ...r,
        invoiceId: ""
      } : r)
    });
  ...
  const updateDisbPayment = (disbId, payId, patch) => {
    ...
    if (dd) upsert("disbursements", {
      ...dd,
      payments: (dd.payments || []).map(p => p.id === payId ? {
        ...p,
        ...patch
      } : p)
    });
  ...
  const getComments = a => {
    ...
    if (a.comment) return [{
      id: "legacy",
      text: a.comment,
      ts: a.date ? new Date(a.date).getTime() : Date.now()
    }];
    return [];
  };
  const saveComments = (agId, list) => {
    ...
    upsert("agreements", {
      ...ag,
      comments: list,
      comment: list.map(c => c.text).join(" · ")
    });
  };
  const addComment = (agId, text) => {
    ...
    const ag = data.agreements.find(x => x.id === agId);
    if (!ag) return;
    saveComments(agId, [...getComments(ag), {
      id: uid(),
      text,
      ts: Date.now()
    }]);
  ...
  const expectedFor = (agId, party) => {
    ...
    return {
      expected,
      paid,
      stillToPay: expected - paid,
      isPaid: list.some(x => x.paymentStatus === "Paid") || paid > 0.005
    };
  };
  ...
  const disbCellsFor = a => {
    ...
    data.disbursements.filter(dd => dd.agreementId === a.id).forEach(dd => {
      let target = rcpts[0];
      if (dd.receiptId && ids.has(dd.receiptId)) target = rcpts.find(r => r.id === dd.receiptId);else rcpts.forEach(r => {
        if (String(r.date || "") <= String(dd.date || "")) target = r;
      });
      const paid = disbPaid(dd);
      const byParty = map[target.id] || (map[target.id] = {});
      const prev = byParty[dd.party] || {
        amount: 0,
        paid: 0,
        statuses: [],
        ids: []
      ...
      byParty[dd.party] = {
        amount: prev.amount + Number(dd.amount || 0),
        paid: prev.paid + paid,
        statuses: [...prev.statuses, dd.paymentStatus || "Ongoing"],
        ids: [...prev.ids, dd.id]
      };
    });
    return map;
  ...
  const allocTot = allocationSummary.reduce((a, r) => ({
  ...
  const buildStatement = party => {
    const rows = [...disbComputed.filter(d => d.party === party).flatMap(d => (d.payments || []).map(p => ({
      date: p.date,
      kind: "in",
      ...
      date: t.date,
      kind: "out",
      desc: `Transfer out — ${t.accountName} (${t.accountCurrency}) · ${t.payType}`,
      currency: t.currency,
    ...
    const withBal = rows.map(r => {
      run += r.kind === "in" ? r.usd : -r.usd;
      return {
        ...r,
        balance: run
      };
    });
    const b = partyBalances[party] || {
      inUSD: 0,
      outUSD: 0
    };
  ...
  const saveAgreementWithAllocs = (a, allocs) => {
    ...
    if (isLocked(a)) {
      save(next);
      setNotice("Agreement saved. Party allocations were not changed because it is closed or archived.");
      return;
    }
  ...
  const loadRegister = () => {
    ...
    if (data.agreements.length) ask("This will REPLACE all current data with the register seed. Continue?", doLoad);else doLoad();
  };
  ...
  const generatePrettyReport = (partyList, preview = false, agIds = null) => {
    ...
    const repAccounts = scoped ? data.accounts.map(acc => {
      const ts = trComputed.filter(t => t.accountId === acc.id && (!partySet || partySet.has(t.fromParty)));
      return {
        ...acc,
        count: ts.length,
        usd: ts.reduce((s, t) => s + t.usd, 0)
      };
    }).filter(a => a.count > 0).sort((a, b) => orderName(a.name, b.name)) : accountTotals;
    ...
    const kpis = agScope ? nativeMode ? [{
      l: "Invoiced",
      primary: nat(invoicedNative),
      sub: usd(invoicedUSDsum),
      c: CK.slate
    }, {
      l: "Received",
      primary: nat(receivedNative),
      sub: usd(kReceived),
      c: CK.emerald
    }, {
      l: "Pending",
      primary: nat(contractNative - receivedNative),
      sub: usd(contractUSDsum - kReceived),
      c: CK.rose
    }, {
      l: "Disbursed",
      primary: nat(kDisbursed / rate),
      sub: usd(kDisbursed),
      c: CK.emerald,
      sep: true
    }] : [{
      l: "Invoiced",
      primary: usd(invoicedUSDsum),
      c: CK.slate
    }, {
      l: "Received",
      primary: usd(kReceived),
      c: CK.emerald
    }, {
      l: "Pending",
      primary: usd(contractUSDsum - kReceived),
      c: CK.rose
    }, {
      l: "Disbursed",
      primary: usd(kDisbursed),
      c: CK.emerald,
      sep: true
    }] : [{
      l: "Received",
      ...
      l: "Undisbursed",
      primary: usd(kReceived - kDisbursed),
      c: CK.slate
    }, {
      l: "Onward to Accounts",
      primary: usd(kOnward),
      c: CK.blue
    }];
    ...
    const agDetail = agScope ? repAg.map(a => {
      ...
      const alT = dl.reduce((s, dd) => ({
        al: s.al + Number(dd.amount || 0),
        pd: s.pd + dd.paidUSD
      }), {
        al: 0,
        pd: 0
      });
      const allocBody = dl.length ? dl.map(dd => {
        const al = Number(dd.amount || 0);
        return `<tr><td>${esc(dd.party)}</td><td class="r">${usd(al)}</td><td class="r">${usd(dd.paidUSD)}</td><td class="r b">${usd(al - dd.paidUSD)}</td></tr>`;
      }).join("") : `<tr><td colspan="4" class="empty">No party allocations.</td></tr>`;
    ...
    const partySections = statementParties.map(p => {
      ...
      const aT = myAlloc.reduce((s, d) => ({
        al: s.al + Number(d.amount || 0),
        pd: s.pd + d.paidUSD
      }), {
        al: 0,
        pd: 0
      });
      const allocTable = myAlloc.length ? `<h3>Allocations by Agreement</h3><table><thead><tr><th>Agreement</th><th class="r">Allocated (USD)</th><th class="r">Paid (USD)</th><th class="r">Pending (USD)</th></tr></thead><tbody>${myAlloc.map(d => {
        const al = Number(d.amount || 0);
        return `<tr><td>${d.agreementTitle}</td><td class="r">${usd(al)}</td><td class="r">${usd(d.paidUSD)}</td><td class="r b">${usd(al - d.paidUSD)}</td></tr>`;
      }).join("")}<tr class="soa-bal"><td><b>Total</b></td><td class="r"><b>${usd(aT.al)}</b></td><td class="r"><b>${usd(aT.pd)}</b></td><td class="r"><b>${usd(aT.al - aT.pd)}</b></td></tr></tbody></table>` : "";
  ...
  const exportExcel = (partyList = [], agIds = null) => {
    ...
    const styleSheet = (ws, headerRowIdx) => {
      ...
      for (let R = range.s.r; R <= range.e.r; R++) for (let C = range.s.c; C <= range.e.c; C++) {
        ...
        const cell = ws[addr];
        if (!cell) continue;
        if (R === headerRowIdx) cell.s = hdr;else if (R > headerRowIdx && (R - headerRowIdx) % 2 === 0) cell.s = {
          ...(cell.s || {}),
          fill: {
            fgColor: {
              rgb: BAND
    ...
    const sum = [["AGREEMENT REVENUE & DISBURSEMENT REPORT (Base Currency: USD)"], [`Generated: ${new Date().toLocaleDateString("en-GB")}`], [`Active filters: ${fLabel}`], [], ["FUNDS FLOW (USD)"], ["Total Received under Agreements", totReceivedUSD], ["Total Disbursed to Parties", totDisbursedUSD], ["Undisbursed Balance", totReceivedUSD - totDisbursedUSD], ["Total Onward Transfers to Accounts", totTransferredUSD], [], ["PER AGREEMENT (USD)"], ["Ref", "Agreement", "Client", "Agreement Status", "Payment Status", "Currency", "Total Value", "Invoiced (USD)", "Outstanding (USD)", "Received (orig.)", "Received (USD)", "Disbursed (USD)", "Undisbursed (USD)", "Comment"], ...xAg.map(a => [a.ref, a.title, a.party, a.status, a.paymentStatus || "Ongoing", a.currency, Number(a.totalValue || 0), a.invoicedUSD, a.outstandingUSD, a.received, a.receivedUSD, a.disbursedUSD, a.undisbursedUSD, a.comment || ""]), [], ["PARTY BALANCES (USD)"], ["Party", "Disbursed In", "Transferred Out", "Balance Held"], ...Object.entries(partyBalances).map(([p, b]) => [p, b.inUSD, b.outUSD, b.inUSD - b.outUSD]), [], ["ACCOUNT TOTALS"], ["Account", "Currency", "Transfers", "Received (own currency)", "Received (USD)", "Comment"], ...accountTotals.map(a => [a.name, a.currency, a.count, a.original, a.usd, a.comment || ""]), [], ["BY YEAR (USD)"], ["Year", "Received", "Disbursed", "Transferred to Accounts"], ...years.map(y => [y, agComputed.flatMap(a => (a.receipts || []).filter(r => yearOf(r.date) === y)).reduce((s, r) => s + r.amount * r.rate, 0), disbComputed.flatMap(d => (d.payments || []).filter(p => yearOf(p.date) === y)).reduce((s, p) => s + p.amount * p.rate, 0), trComputed.filter(t => yearOf(t.date) === y).reduce((s, t) => s + t.usd, 0)])];
    ...
    addStyledSheet("Agreements", xAg.map(a => ({
      Ref: a.ref,
      Title: a.title,
      Client: a.party,
      Date: a.date,
      "Agreement Status": a.status,
      "Payment Status": a.paymentStatus || "Ongoing",
      Currency: a.currency,
      "Total Value": Number(a.totalValue || 0),
      "Invoiced (orig.)": a.invoiced,
      "Invoiced (USD)": a.invoicedUSD,
      "Outstanding (USD)": a.outstandingUSD,
      "Received (orig.)": a.received,
      "Received (USD)": a.receivedUSD,
      "Disbursed (USD)": a.disbursedUSD,
      "Undisbursed (USD)": a.undisbursedUSD,
      Comment: a.comment || ""
    })));
    addStyledSheet("Invoices", xAg.flatMap(a => (a.invoices || []).map(iv => ({
      "Invoice Date": iv.date,
      "Invoice #": iv.number || "",
      "Due Date": iv.dueDate || "",
      Agreement: a.title,
      Ref: a.ref,
      ...
      "Rate to USD": Number(iv.rate || 1),
      "USD Equivalent": Number(iv.amount || 0) * Number(iv.rate || 1),
      Received: (a.receipts || []).filter(r => r.invoiceId === iv.id).reduce((t, r) => t + Number(r.amount || 0), 0),
      Comment: iv.notes || ""
    }))));
    addStyledSheet("Receipts", xAg.flatMap(a => (a.receipts || []).map(r => ({
      Date: r.date,
      Agreement: a.title,
      Ref: a.ref,
      Client: a.party,
      ...
      "Rate to USD": Number(r.rate),
      "USD Equivalent": r.amount * r.rate,
      Comment: r.notes || ""
    }))));
    addStyledSheet("Disbursements", xDisb.map(d => ({
      Date: d.date,
      Party: d.party,
    ...
    addStyledSheet("Disb Payments", xDisb.flatMap(d => (d.payments || []).map(p => ({
      Date: p.date,
      Party: d.party,
      "Source Agreement": d.agreementTitle,
      Currency: p.currency || d.currency,
    ...
    addStyledSheet("Transfers", xTr.map(t => ({
      Date: t.date,
      "From Party": t.fromParty,
      "To Account": t.accountName,
      "Account Currency": t.accountCurrency,
      "Partial/Full": t.payType,
      "Transfer Currency": t.currency,
      Amount: Number(t.amount),
      "Rate to USD": Number(t.rate),
  ...
  if (LOGIN_ENABLED && !authed && !syncChecked && users.length === 0) return /*#__PURE__*/React.createElement("div", {
    className: "min-h-screen bg-slate-100 flex items-center justify-center text-slate-500"
  }, "Connecting…");
  ...
  const Sel = ({
    ...
    className: "shrink-0 border border-slate-200 rounded-md px-2.5 py-1.5 text-xs bg-white text-slate-700 max-w-[200px] shadow-sm hover:border-slate-300 focus:border-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-100 transition-colors"
  }, /*#__PURE__*/React.createElement("option", {
    value: "all"
  }, label, ": All"), opts.map((o, i) => /*#__PURE__*/React.createElement("option", {
    key: o,
    value: o
  }, names ? names[i] : o)));
  ...
  const Badge = ({
    ...
    className: `inline-flex items-center justify-center gap-1.5 min-w-[72px] px-2 py-0.5 rounded-md text-[11px] font-medium ring-1 ring-inset ${statusColor(s)}`
  }, /*#__PURE__*/React.createElement("span", {
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "w-9 h-9 sm:w-11 sm:h-11 shrink-0 rounded-md border border-slate-700 bg-slate-900/50 flex items-center justify-center font-serif text-base sm:text-lg tracking-widest text-slate-100"
  }, "X"), /*#__PURE__*/React.createElement("div", {
    className: "border-l border-slate-700/70 pl-2.5 sm:pl-4 min-w-0"
  }, /*#__PURE__*/React.createElement("h1", {
    className: "text-base sm:text-xl font-serif tracking-wide text-white truncate"
  }, "XYZ Financial Report"), /*#__PURE__*/React.createElement("p", {
    className: "text-[9px] sm:text-[11px] uppercase tracking-[0.14em] sm:tracking-[0.18em] text-slate-400 mt-0.5 truncate"
  }, "Agreement Revenue & Disbursement", /*#__PURE__*/React.createElement("span", {
  ...
  }, "As of"), /*#__PURE__*/React.createElement("div", {
    ...
    className: "h-10 w-px bg-slate-700/70 hidden sm:block"
  }), /*#__PURE__*/React.createElement("div", {
    className: "no-print flex gap-1.5"
  }, /*#__PURE__*/React.createElement("button", {
    onClick: printView,
    title: "Print as PDF",
    className: "w-8 h-8 flex items-center justify-center rounded-md border border-slate-700 hover:border-slate-400 hover:bg-slate-800 text-slate-300 transition-colors"
    ...
    className: "w-8 h-8 flex items-center justify-center rounded-md bg-emerald-600 hover:bg-emerald-500 text-white transition-colors"
  }, /*#__PURE__*/React.createElement("svg", {
    width: "14",
    height: "14",
    viewBox: "0 0 24 24",
    ...
    onClick: logout,
    title: `Logout — ${users.find(u => u.userId === currentUser)?.name || currentUser || ""} · ${currentUser || ""} (${roleLabel(users.find(u => u.userId === currentUser)?.role)})`,
    className: "w-8 h-8 flex items-center justify-center rounded-md border border-slate-700 hover:border-rose-500 hover:bg-slate-800 text-slate-300 hover:text-rose-400 transition-colors"
  }, /*#__PURE__*/React.createElement("svg", {
    width: "14",
    height: "14",
    viewBox: "0 0 24 24",
    ...
    onClick: () => {
      setTab(k);
      if (k !== "parties") setSelectedParty(null);
    },
    ...
    className: "text-[10px] font-semibold text-slate-400 uppercase tracking-[0.18em] mr-2 shrink-0"
  }, "Filters"), /*#__PURE__*/React.createElement(Sel, {
    k: "agreement",
    label: "Agreement",
    opts: data.agreements.map(a => a.id),
    names: data.agreements.map(a => (a.ref ? a.ref + " · " : "") + a.title)
    ...
    k: "currency",
    label: "Currency",
    opts: data.currencies.map(c => c.code)
  }), /*#__PURE__*/React.createElement(Sel, {
    k: "agStatus",
    ...
    opts: PAY_STATUSES
  }), Object.values(F).some(v => v !== "all") && /*#__PURE__*/React.createElement("button", {
    onClick: () => setFilters({
      party: "all",
      year: "all",
      currency: "all",
  ...
  })), /*#__PURE__*/React.createElement("b", null, "Read-only copy."), /*#__PURE__*/React.createElement("span", null, "Release ", release?.version, " locked data entry to the published app", release?.host ? ` (${release.host})` : "", ". You can view, filter and export here, but edits will not save."))), /*#__PURE__*/React.createElement(ErrorBoundary, null, /*#__PURE__*/React.createElement("div", {
    className: "w-full px-3 sm:px-6 lg:px-8 xl:px-10 py-4 sm:py-6"
  }, tab === "dashboard" && /*#__PURE__*/React.createElement("div", {
    className: "space-y-4"
  }, data.agreements.length === 0 && /*#__PURE__*/React.createElement("div", {
    className: "bg-gradient-to-r from-blue-50 to-blue-50/40 border border-blue-200 rounded-xl p-5 flex flex-wrap items-center gap-4 shadow-sm"
  }, /*#__PURE__*/React.createElement("div", {
    className: "flex-1 min-w-[220px]"
  }, /*#__PURE__*/React.createElement("p", {
    className: "font-semibold text-blue-900"
  }, "Load the Invoicing & Allocation Register"), /*#__PURE__*/React.createElement("p", {
    className: "text-sm text-blue-700"
  }, "Populate the tracker with all agreements, received invoices, and per-party allocations.")), /*#__PURE__*/React.createElement("button", {
    onClick: loadRegister,
    className: "bg-blue-700 hover:bg-blue-600 text-white text-sm font-medium px-4 py-2 rounded-lg shadow-sm transition-colors"
  }, "Load Register Data")), /*#__PURE__*/React.createElement("div", {
    className: "bg-white rounded-xl border border-slate-200 shadow-sm divide-y sm:divide-y-0 sm:divide-x divide-slate-200 grid grid-cols-1 sm:grid-cols-3"
  }, [{
    l: "Total Invoiced · USD",
    v: totalInvoicedUSD,
    sym: "$",
    dot: "bg-emerald-500"
  }, {
    l: "Total Invoiced · EUR",
    v: totalInvoicedEUR,
    sym: "€",
    dot: "bg-blue-500"
  }, {
    l: "Total Invoiced · AED",
    v: totalInvoicedAED,
    sym: "AED",
    dot: "bg-amber-500"
    ...
    className: `w-1.5 h-1.5 rounded-full ${x.dot}`
  }), /*#__PURE__*/React.createElement("span", {
    className: "text-[9px] uppercase tracking-[0.16em] text-slate-400 font-semibold"
  }, x.l)), /*#__PURE__*/React.createElement("div", {
    className: "text-lg text-slate-900 font-medium mt-1"
  }, /*#__PURE__*/React.createElement(AmtTile, {
    v: x.v,
    sym: x.sym
  }))))), /*#__PURE__*/React.createElement("div", {
    className: "bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm"
  }, /*#__PURE__*/React.createElement("div", {
    className: "px-4 sm:px-6 lg:px-8 py-3 sm:py-4 border-b border-slate-200 flex items-baseline justify-between flex-wrap gap-2"
  }, /*#__PURE__*/React.createElement("h3", {
    className: "font-serif text-lg sm:text-xl text-slate-900 tracking-tight"
  }, "Agreements (Funds Flow)"), /*#__PURE__*/React.createElement("span", {
    className: "text-[9px] uppercase tracking-[0.2em] text-slate-400"
  }, "All amounts in USD unless noted")), /*#__PURE__*/React.createElement("div", {
    className: "grid grid-cols-2 md:grid-cols-4 gap-2.5 px-4 sm:px-6 lg:px-8 pt-3.5 pb-1"
  }, [{
    l: "Received",
    v: totReceivedUSD,
    c: "text-emerald-700",
    ...
    l: "Disbursed to Parties",
    v: totDisbursedUSD,
    c: "text-rose-700",
    bar: "bg-rose-500"
  }, {
    l: "Undisbursed Balance",
    v: totReceivedUSD - totDisbursedUSD,
    c: "text-slate-900",
    bar: "bg-slate-700"
  }, {
    l: "Onward to Accounts",
    ...
    key: x.l,
    className: "relative bg-white rounded-lg border border-slate-200 px-3 py-2.5 overflow-hidden"
  }, /*#__PURE__*/React.createElement("span", {
    className: `absolute top-0 left-0 right-0 h-0.5 ${x.bar}`
  }), /*#__PURE__*/React.createElement("p", {
    className: "text-[9px] uppercase tracking-[0.16em] text-slate-400 font-medium truncate"
  }, x.l), /*#__PURE__*/React.createElement("p", {
    className: `text-base mt-0.5 ${Number(x.v) < -0.005 ? "text-rose-600" : x.c}`
  }, /*#__PURE__*/React.createElement(AmtTile, {
    v: x.v
  }))))), /*#__PURE__*/React.createElement("div", {
  ...
  }, "Agreement Value"), /*#__PURE__*/React.createElement("th", {
    className: "text-right px-4 py-2 font-semibold"
  }, "Invoiced"), /*#__PURE__*/React.createElement("th", {
    className: "text-right px-4 py-2 font-semibold"
  }, "Total Received"), /*#__PURE__*/React.createElement("th", {
    className: "text-right px-4 py-2 font-semibold"
  }, "Pending"), /*#__PURE__*/React.createElement("th", {
    className: "text-right px-4 py-2 font-semibold"
  }, "Disbursed"), /*#__PURE__*/React.createElement("th", {
    className: "text-right px-4 py-2 font-semibold"
  }, "Undisbursed"), /*#__PURE__*/React.createElement("th", {
    className: "text-center px-2 py-3 font-semibold w-10"
    ...
    return /*#__PURE__*/React.createElement(Fragment, {
      ...
      className: `border-t border-slate-100 hover:bg-slate-50 transition-colors ${i % 2 ? "bg-slate-50/30" : ""}`
    }, /*#__PURE__*/React.createElement("td", {
      className: "px-4 py-2 text-slate-400 font-medium tabular-nums"
    }, String(agNum[a.id]).padStart(2, "0")), /*#__PURE__*/React.createElement("td", {
      className: "px-4 py-3"
    }, /*#__PURE__*/React.createElement("div", {
      className: "font-medium text-slate-800"
    }, a.title), a.party && a.party !== a.title && /*#__PURE__*/React.createElement("div", {
      className: "text-[11px] text-slate-400 mt-0.5"
    }, a.party)), /*#__PURE__*/React.createElement("td", {
      className: "px-4 py-3"
    }, /*#__PURE__*/React.createElement(Badge, {
      s: a.status
    })), /*#__PURE__*/React.createElement("td", {
      className: "px-4 py-2 text-right whitespace-nowrap text-slate-600"
    }, /*#__PURE__*/React.createElement(AmtG, {
      v: a.totalValue,
      sym: csym(a.currency)
    })), /*#__PURE__*/React.createElement("td", {
      className: "px-4 py-2 text-right whitespace-nowrap text-slate-500"
    }, /*#__PURE__*/React.createElement(AmtG, {
      v: a.invoiced,
      sym: csym(a.currency)
    })), /*#__PURE__*/React.createElement("td", {
      className: "px-4 py-2 text-right text-emerald-700 font-medium"
    }, /*#__PURE__*/React.createElement(AmtG, {
      v: a.receivedUSD
    })), /*#__PURE__*/React.createElement("td", {
      className: `px-4 py-3 text-right ${pending > 0.01 ? "text-rose-400" : "text-slate-300"}`
    }, /*#__PURE__*/React.createElement(AmtG, {
      v: pending
    })), /*#__PURE__*/React.createElement("td", {
      className: "px-4 py-2 text-right text-emerald-500 font-medium"
    }, /*#__PURE__*/React.createElement(AmtG, {
      v: a.disbursedUSD
    })), /*#__PURE__*/React.createElement("td", {
      className: `px-4 py-3 text-right font-medium ${a.undisbursedUSD < -0.01 ? "text-rose-500" : a.undisbursedUSD > 0.01 ? "text-rose-300" : "text-slate-300"}`
    }, /*#__PURE__*/React.createElement(AmtG, {
      v: a.undisbursedUSD
    })), /*#__PURE__*/React.createElement("td", {
      ...
      onClick: e => {
        e.stopPropagation();
        setCommentRow(commentRow === a.id ? null : a.id);
        setNewComment("");
      },
      title: getComments(a).length ? `${getComments(a).length} comment(s)` : "Add comment",
      className: `relative inline-flex items-center justify-center w-7 h-7 rounded-md border transition-colors ${getComments(a).length ? "text-blue-600 border-blue-200 bg-blue-50" : "text-slate-300 border-slate-200 hover:text-slate-500 hover:border-slate-300"}`
    }, /*#__PURE__*/React.createElement("svg", {
      width: "13",
      height: "13",
      viewBox: "0 0 24 24",
    ...
    })), getComments(a).length > 0 && /*#__PURE__*/React.createElement("span", {
      className: "absolute -top-1.5 -right-1.5 bg-blue-600 text-white text-[8px] font-semibold w-3.5 h-3.5 flex items-center justify-center rounded-full"
    }, getComments(a).length)))), commentRow === a.id && /*#__PURE__*/React.createElement("tr", {
      className: "bg-slate-50"
    }, /*#__PURE__*/React.createElement("td", {
      colSpan: 10,
      className: "px-4 py-2 border-t border-slate-100"
    }, /*#__PURE__*/React.createElement("div", {
      className: "max-w-2xl"
    }, /*#__PURE__*/React.createElement("div", {
    ...
    }, getComments(a).slice().sort((x, y) => x.ts - y.ts).map(m => /*#__PURE__*/React.createElement("div", {
      key: m.id,
      ...
      className: "bg-slate-900 hover:bg-slate-700 text-white text-xs px-3 py-1.5 rounded-md"
    }, "Post"))))));
  })), fAg.length > 0 && (() => {
    const t = fAg.reduce((s, a) => {
      ...
      return {
        val: s.val + v,
        recv: s.recv + a.receivedUSD,
        pend: s.pend + ((a.hasInvoices ? a.invoicedUSD : v) - a.receivedUSD),
        disb: s.disb + a.disbursedUSD,
        undisb: s.undisb + a.undisbursedUSD
      };
    }, {
      val: 0,
      recv: 0,
    ...
    return /*#__PURE__*/React.createElement("tfoot", {
      className: "bg-slate-100 border-t-2 border-slate-300 text-slate-800"
    }, /*#__PURE__*/React.createElement("tr", {
      className: "text-[11px] uppercase tracking-wider font-semibold"
    }, /*#__PURE__*/React.createElement("td", {
      colSpan: 3,
      className: "px-4 py-3"
    }, "Total (", fAg.length, " agreements)"), /*#__PURE__*/React.createElement("td", {
      className: "px-4 py-2 text-right"
    }, /*#__PURE__*/React.createElement(AmtG, {
      v: t.val
    })), /*#__PURE__*/React.createElement("td", null), /*#__PURE__*/React.createElement("td", {
      className: "px-4 py-2 text-right text-emerald-700"
    }, /*#__PURE__*/React.createElement(AmtG, {
      v: t.recv
    })), /*#__PURE__*/React.createElement("td", {
      ...
      className: "px-4 py-2 text-right text-emerald-600"
    }, /*#__PURE__*/React.createElement(AmtG, {
      v: t.disb
    })), /*#__PURE__*/React.createElement("td", {
      className: "px-4 py-2 text-right text-rose-500"
    ...
    className: "flex items-start justify-between mb-1 flex-wrap gap-2"
  }, /*#__PURE__*/React.createElement("h2", {
    className: "font-serif text-2xl text-slate-900 tracking-tight"
  }, "Allocation Summary"), /*#__PURE__*/React.createElement("div", {
    className: "text-right"
  }, /*#__PURE__*/React.createElement("span", {
    className: "block text-[10px] uppercase tracking-[0.2em] text-slate-400"
  }, "As of ", new Date().toLocaleDateString("en-GB", {
    day: "2-digit",
    month: "long",
    year: "numeric"
  })), /*#__PURE__*/React.createElement("div", {
    className: "flex justify-end gap-x-5 mt-1.5 text-[11px]"
  }, /*#__PURE__*/React.createElement("span", null, /*#__PURE__*/React.createElement("span", {
    className: "uppercase tracking-[0.12em] text-[9px] text-slate-400 mr-1.5"
  }, "AED/USD"), /*#__PURE__*/React.createElement("span", {
    className: "text-slate-700 tabular-nums"
  }, "3.65")), /*#__PURE__*/React.createElement("span", null, /*#__PURE__*/React.createElement("span", {
    className: "uppercase tracking-[0.12em] text-[9px] text-slate-400 mr-1.5"
  }, "EUR/USD"), /*#__PURE__*/React.createElement("span", {
    className: "text-slate-700 tabular-nums"
    ...
    className: "text-left px-4 py-2 font-semibold"
  }, "Allocation"), /*#__PURE__*/React.createElement("th", {
    className: "text-left px-4 py-2 font-semibold"
  }, "Detail"), /*#__PURE__*/React.createElement("th", {
    className: "text-right px-4 py-2 font-semibold"
  }, "Invoiced (USD)"), /*#__PURE__*/React.createElement("th", {
    className: "text-right px-4 py-2 font-semibold"
  }, "Paid (USD)"), /*#__PURE__*/React.createElement("th", {
    className: "text-right px-4 py-2 font-semibold"
  }, "Pending (USD)"))), /*#__PURE__*/React.createElement("tbody", {
    className: "text-slate-700"
    ...
    className: `px-4 py-3 text-right ${r.pending < 0 ? "text-rose-600" : r.pending > 0.01 ? "text-rose-400" : "text-slate-300"}`
  }, /*#__PURE__*/React.createElement(AmtG, {
    v: r.pending
  })))), allocationSummary.length > 0 && /*#__PURE__*/React.createElement("tr", {
    className: "border-t-2 border-slate-300 bg-slate-50 font-semibold text-slate-900"
  }, /*#__PURE__*/React.createElement("td", {
    className: "px-4 py-3.5 text-[10px] uppercase tracking-[0.14em]",
    colSpan: 2
    ...
    className: "px-4 py-3.5 text-right text-emerald-700"
  }, /*#__PURE__*/React.createElement(AmtG, {
    v: allocTot.paid
  })), /*#__PURE__*/React.createElement("td", {
    className: `px-4 py-3.5 text-right ${allocTot.pending < 0 ? "text-rose-600" : "text-slate-900"}`
  }, /*#__PURE__*/React.createElement(AmtG, {
    v: allocTot.pending
  }))))))), /*#__PURE__*/React.createElement("div", {
    className: "bg-white rounded-xl border border-slate-200 overflow-hidden shadow-sm"
    ...
    className: "text-left px-5 py-2 font-semibold"
    ...
    className: "text-right px-5 py-2 font-semibold"
  }, "Paid Onward"), /*#__PURE__*/React.createElement("th", {
    className: "text-right px-5 py-2 font-semibold"
  }, "Holding"))), /*#__PURE__*/React.createElement("tbody", {
    className: "text-slate-700"
  }, Object.keys(partyBalances).length === 0 && /*#__PURE__*/React.createElement("tr", null, /*#__PURE__*/React.createElement("td", {
    colSpan: 4,
    className: "px-5 py-8 text-center text-slate-400 italic"
  }, "No disbursements yet.")), Object.entries(partyBalances).sort((a, b2) => orderParty(a[0], b2[0])).map(([p, b], i) => /*#__PURE__*/React.createElement("tr", {
    key: p,
    className: `border-t border-slate-100 hover:bg-slate-50/80 transition-colors ${i % 2 ? "bg-slate-50/30" : ""}`
  }, /*#__PURE__*/React.createElement("td", {
    ...
    v: b.inUSD - b.outUSD
  }))))))))), tab === "agreements" && /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    className: "flex flex-wrap justify-between items-baseline gap-3 mb-6"
  }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("h2", {
    className: "font-serif text-2xl text-slate-900 tracking-tight"
  }, "Agreements & Receipts"), /*#__PURE__*/React.createElement("p", {
    className: "text-[10px] uppercase tracking-[0.18em] text-slate-400 mt-1"
  }, agPageList.length, " entries")), /*#__PURE__*/React.createElement("div", {
    className: "flex flex-wrap items-center gap-2"
  }, agComputed.some(a => a.archived) && /*#__PURE__*/React.createElement("button", {
    onClick: () => setShowArchived(!showArchived),
    className: `text-[10px] uppercase tracking-[0.14em] px-3 py-2 rounded-lg border transition-colors ${showArchived ? "bg-slate-800 text-white border-slate-800" : "bg-white text-slate-600 border-slate-200 hover:border-slate-400"}`
  }, showArchived ? "Hide" : "Show", " archived (", agComputed.filter(a => a.archived).length, ")"), agComputed.some(a => a.hidden) && /*#__PURE__*/React.createElement("button", {
    onClick: () => setShowHidden(!showHidden),
    className: `text-[10px] uppercase tracking-[0.14em] px-3 py-2 rounded-lg border transition-colors ${showHidden ? "bg-slate-800 text-white border-slate-800" : "bg-white text-slate-600 border-slate-200 hover:border-slate-400"}`
  }, showHidden ? "Hide" : "Show", " hidden (", agComputed.filter(a => a.hidden).length, ")"), /*#__PURE__*/React.createElement("button", {
    onClick: () => setAgOpen({}),
    className: "text-[10px] uppercase tracking-[0.14em] px-3 py-2 rounded-lg border border-slate-200 text-slate-600 bg-white hover:border-slate-400 transition-colors"
  }, "Collapse all"), /*#__PURE__*/React.createElement("button", {
    onClick: () => setAgOpen(Object.fromEntries(agPageList.map(a => [a.id, true]))),
    className: "text-[10px] uppercase tracking-[0.14em] px-3 py-2 rounded-lg border border-slate-200 text-slate-600 bg-white hover:border-slate-400 transition-colors"
    ...
    className: "bg-slate-900 hover:bg-slate-700 text-white text-[11px] uppercase tracking-[0.14em] px-4 py-2 rounded-lg shadow-sm transition-colors"
  }, "+ New Agreement"))), /*#__PURE__*/React.createElement("div", {
    className: "space-y-3"
  }, agPageList.length === 0 && /*#__PURE__*/React.createElement("div", {
    className: "bg-white rounded-xl border border-slate-200 p-8 text-center text-slate-400 shadow-sm"
  }, "No agreements match the current filters."), agPageList.map(a => /*#__PURE__*/React.createElement("div", {
    key: a.id,
    className: `bg-white rounded-xl border shadow-sm transition-shadow hover:shadow-md ${a.hidden ? "border-slate-300 opacity-60" : a.archived ? "border-amber-200" : "border-slate-200"}`
  }, /*#__PURE__*/React.createElement("div", {
    ...
    className: "text-slate-400 font-medium tabular-nums text-sm"
  }, String(agNum[a.id]).padStart(2, "0")), /*#__PURE__*/React.createElement("span", {
    className: "text-base text-slate-900 truncate"
  }, a.title)), /*#__PURE__*/React.createElement(Badge, {
    s: a.status
  }), /*#__PURE__*/React.createElement(Badge, {
    s: a.paymentStatus || "Ongoing",
    prefix: "Pay"
  }), a.archived && /*#__PURE__*/React.createElement("span", {
    className: "text-[9px] uppercase tracking-[0.18em] px-2 py-1 rounded-full bg-amber-50 text-amber-700 border border-amber-200"
  }, "Archived"), a.hidden && /*#__PURE__*/React.createElement("span", {
    className: "text-[9px] uppercase tracking-[0.18em] px-2 py-1 rounded-full bg-slate-100 text-slate-500 border border-slate-200"
  }, "Hidden"), isLocked(a) && /*#__PURE__*/React.createElement("span", {
    className: "inline-flex items-center gap-1 text-[9px] uppercase tracking-[0.18em] px-2 py-1 rounded-full bg-slate-800 text-white"
  }, /*#__PURE__*/React.createElement("svg", {
    width: "8",
    height: "8",
    viewBox: "0 0 24 24",
    ...
    className: "text-[9px] uppercase tracking-wide text-slate-400 leading-none mb-0.5"
  }, "Contract Value"), /*#__PURE__*/React.createElement("div", {
    className: "text-sm font-bold text-slate-800 leading-tight mb-1"
  }, a.currency, " ", fmt(a.totalValue)), /*#__PURE__*/React.createElement("div", {
    className: "flex flex-col items-end gap-0.5 text-[10px] min-w-[160px]"
  }, /*#__PURE__*/React.createElement("span", {
    className: "flex items-center justify-between gap-2 px-2 py-0.5 rounded bg-slate-100 text-slate-600 border border-slate-200 w-full"
  }, /*#__PURE__*/React.createElement("span", {
    className: "opacity-80 uppercase tracking-wide text-[8px]"
  ...
  }, fmt(a.invoiced)), /*#__PURE__*/React.createElement("span", {
    className: "text-[8px] opacity-70 ml-1"
  }, a.currency))), /*#__PURE__*/React.createElement("span", {
    className: "flex items-center justify-between gap-2 px-2 py-0.5 rounded bg-emerald-50 text-emerald-700 border border-emerald-100 w-full"
  }, /*#__PURE__*/React.createElement("span", {
    className: "opacity-80 uppercase tracking-wide text-[8px]"
  ...
  }, fmt(a.received)), /*#__PURE__*/React.createElement("span", {
    className: "text-[8px] opacity-70 ml-1"
  }, a.currency))), /*#__PURE__*/React.createElement("span", {
    className: "flex items-center justify-between gap-2 px-2 py-0.5 rounded bg-emerald-50/60 text-emerald-500 border border-emerald-100 w-full"
  }, /*#__PURE__*/React.createElement("span", {
    className: "opacity-80 uppercase tracking-wide text-[8px]"
  }, "Disbursed"), /*#__PURE__*/React.createElement("span", {
    className: "whitespace-nowrap"
  }, /*#__PURE__*/React.createElement("b", {
    className: "tabular-nums inline-block text-right",
    style: {
  ...
  }, fmt(a.disbursedUSD)), /*#__PURE__*/React.createElement("span", {
    className: "text-[8px] opacity-70 ml-1"
  }, "USD"))), /*#__PURE__*/React.createElement("span", {
    className: "flex items-center justify-between gap-2 px-2 py-0.5 rounded bg-rose-50 text-rose-400 border border-rose-100 w-full"
  }, /*#__PURE__*/React.createElement("span", {
    className: "opacity-80 uppercase tracking-wide text-[8px]"
  ...
  }, fmt(Number(a.totalValue || 0) * (FX[a.currency] || 1) - a.receivedUSD)), /*#__PURE__*/React.createElement("span", {
    className: "text-[8px] opacity-70 ml-1"
  }, "USD"))))), /*#__PURE__*/React.createElement("span", {
    className: "text-slate-400 self-center"
  }, agOpen[a.id] ? "▲" : "▼")), agOpen[a.id] && /*#__PURE__*/React.createElement("div", {
    className: "border-t border-slate-100 px-4 py-3 bg-slate-50/70 rounded-b-xl"
  }, /*#__PURE__*/React.createElement("div", {
    className: "overflow-x-auto"
  }, (() => {
    ...
    invs.forEach(iv => {
      ...
      if (!mine.length) rows.push({
        key: "i" + iv.id,
        iv,
        r: null,
        cont: false
      });else mine.forEach((r, i) => rows.push({
        key: r.id,
        iv,
        r,
        cont: i > 0
    ...
    rcpts.filter(r => !r.invoiceId || !invIds.has(r.invoiceId)).forEach(r => rows.push({
      key: r.id,
      iv: null,
      r,
      cont: false
    ...
    return /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("table", {
      className: "text-sm bg-white rounded-lg border border-slate-200 overflow-hidden",
      style: {
        minWidth: 780 + P * 84,
        width: "100%"
      }
    }, /*#__PURE__*/React.createElement("thead", {
      className: "text-xs text-slate-500 uppercase bg-slate-100"
    }, /*#__PURE__*/React.createElement("tr", {
      className: "text-[10px] tracking-[0.16em] text-slate-400"
    }, /*#__PURE__*/React.createElement("th", {
      colSpan: 3,
      className: "text-left px-3 pt-2 pb-1 border-r border-slate-200"
    }, "Invoice"), /*#__PURE__*/React.createElement("th", {
      colSpan: 3,
      className: `text-left px-3 pt-2 pb-1 ${P ? "border-r border-slate-200" : ""}`
    }, "Payments Received"), P > 0 && /*#__PURE__*/React.createElement("th", {
      colSpan: P,
      className: "text-left px-2 pt-2 pb-1 border-r border-slate-200"
    }, "Allocations"), /*#__PURE__*/React.createElement("th", null)), /*#__PURE__*/React.createElement("tr", null, /*#__PURE__*/React.createElement("th", {
      className: "text-left px-3 pb-1.5"
    }, "Invoice Date"), /*#__PURE__*/React.createElement("th", {
      className: "text-left px-3 pb-1.5"
    }, "Invoice #"), /*#__PURE__*/React.createElement("th", {
      className: "text-right px-3 pb-1.5 border-r border-slate-200"
    }, "Amount (", a.currency, ")"), /*#__PURE__*/React.createElement("th", {
      className: "text-left px-3 pb-1.5"
    }, "Receipt Date"), /*#__PURE__*/React.createElement("th", {
      className: "text-right px-3 pb-1.5"
    }, "Amount (", a.currency, ")"), /*#__PURE__*/React.createElement("th", {
      className: `text-right px-2 pb-1.5 w-16 ${P ? "border-r border-slate-200" : ""}`
    }, "Rate"), agParties.map((p, i) => {
      const ex = expectedFor(a.id, p);
      return /*#__PURE__*/React.createElement("th", {
        key: p,
        className: `text-right px-2 pb-1.5 whitespace-nowrap align-bottom ${i === P - 1 ? "border-r border-slate-200" : ""}`
      }, /*#__PURE__*/React.createElement("span", {
        className: "block"
      }, p), /*#__PURE__*/React.createElement("span", {
        title: `${p} — expected ${fmt(ex.expected)} · disbursed ${fmt(ex.paid)} · still to pay ${fmt(ex.stillToPay)}`,
        className: `block text-[10px] font-normal normal-case tracking-normal tabular-nums ${ex.isPaid ? "text-emerald-600" : "text-slate-300"}`
      }, fmt(ex.expected)));
    }), /*#__PURE__*/React.createElement("th", {
      className: "w-10"
    ...
    }, "No invoices or payments recorded yet.")), rows.map(({
      key,
      iv,
      r,
      cont
      ...
      return /*#__PURE__*/React.createElement("tr", {
      ...
      }, overdue ? "Overdue" : "Awaiting payment", iv && iv.dueDate ? ` · due ${iv.dueDate}` : "") : isLocked(a) ? r.date : /*#__PURE__*/React.createElement("button", {
        onClick: () => setModal({
          type: "receipt",
          payload: {
            agreement: a,
        ...
        className: "px-3 py-1.5 text-right whitespace-nowrap text-emerald-700"
      }, !r ? "" : isLocked(a) ? `${csym(a.currency)} ${fmt(r.amount)}` : /*#__PURE__*/React.createElement("button", {
        onClick: () => setModal({
          type: "receipt",
          payload: {
            agreement: a,
      ...
      }, csym(a.currency), " ", fmt(r.amount))), /*#__PURE__*/React.createElement("td", {
        className: `px-3 py-1.5 text-right ${P ? "border-r border-slate-100" : ""}`
      }, !r ? "" : isLocked(a) ? Number(r.rate).toFixed(2) : /*#__PURE__*/React.createElement("button", {
        onClick: () => setModal({
          type: "receipt",
          payload: {
            agreement: a,
        ...
        title: "Edit this payment",
        className: "tabular-nums hover:text-blue-700 hover:underline"
      }, Number(r.rate).toFixed(2))), agParties.map((p, i) => {
        ...
        const open = () => setModal({
          ...
          payload: {
            ...
            receipt: r,
            party: p,
            disbId: cell && !many ? cell.ids[0] : null
          }
        });
        return /*#__PURE__*/React.createElement("td", {
          key: p,
          title: !r ? undefined : cell ? `${p} — disbursement of ${fmt(cell.amount)} · paid ${fmt(cell.paid)} · ${cell.statuses.join(", ")}${many ? " · several records" : ""}. Click to edit.` : `No disbursement to ${p} against this payment. Click to add one.`,
          ...
          onClick: open,
          className: "inline-flex items-baseline justify-end gap-0.5 w-full rounded px-1 py-0.5 border border-transparent hover:border-slate-200 hover:bg-slate-50"
        }, /*#__PURE__*/React.createElement("span", null, fmt(cell ? cell.paid : 0)), /*#__PURE__*/React.createElement("span", {
          className: "opacity-50"
        }, "$")));
      }), /*#__PURE__*/React.createElement("td", {
        className: "px-2 py-1.5 text-center"
      }, !r ? iv && iv.notes ? /*#__PURE__*/React.createElement("span", {
        title: iv.notes,
        ...
        onBlur: e => {
          updateReceipt(a.id, r.id, {
            comment: e.target.value
          });
          setRcptCommentEdit(null);
        },
        onKeyDown: e => {
        ...
        title: r.comment || "Add comment",
        className: `inline-flex items-center justify-center w-6 h-6 rounded-md border transition-colors ${r.comment ? "text-blue-600 border-blue-200 bg-blue-50" : "text-slate-300 border-slate-200 hover:text-slate-500 hover:border-slate-300"}`
      }, /*#__PURE__*/React.createElement("svg", {
        width: "11",
        height: "11",
        viewBox: "0 0 24 24",
      ...
      className: "text-[11px] uppercase tracking-wider font-semibold text-slate-700"
    }, /*#__PURE__*/React.createElement("td", {
      colSpan: 2,
      ...
      className: "px-3 py-2 text-right whitespace-nowrap text-emerald-700"
    }, csym(a.currency), " ", fmt(a.received)), /*#__PURE__*/React.createElement("td", {
      className: P ? "border-r border-slate-200" : ""
    }), agParties.map((p, i) => /*#__PURE__*/React.createElement("td", {
      key: p,
    ...
    className: "flex items-center gap-1.5 text-xs text-slate-500 bg-white border border-slate-200 rounded-lg px-3 py-1.5"
  }, /*#__PURE__*/React.createElement("svg", {
    width: "11",
    height: "11",
    viewBox: "0 0 24 24",
    ...
    className: "bg-slate-900 hover:bg-slate-700 text-white text-xs px-3 py-1.5 rounded-lg shadow-sm"
  }, "+ Record Invoice"), /*#__PURE__*/React.createElement("button", {
    onClick: () => setModal({
      type: "receipt",
      payload: {
        agreement: a,
    ...
    className: "bg-emerald-600 hover:bg-emerald-500 text-white text-xs px-3 py-1.5 rounded-lg shadow-sm"
  }, "+ Record Payment Received"), /*#__PURE__*/React.createElement("button", {
    onClick: () => setModal({
      type: "disbursement",
      payload: {
        presetAgreement: a.id
      }
    }),
    className: "bg-rose-600 hover:bg-rose-500 text-white text-xs px-3 py-1.5 rounded-lg shadow-sm"
  }, "+ Disburse from this Agreement")), /*#__PURE__*/React.createElement("button", {
    onClick: () => setModal({
      type: "agreement",
      payload: a
    }),
    className: "text-blue-700 text-xs px-2"
  }, "Edit"), /*#__PURE__*/React.createElement("button", {
    onClick: () => upsert("agreements", {
      ...data.agreements.find(x => x.id === a.id),
      archived: !a.archived
    }),
    className: "text-amber-700 text-xs px-2"
  }, a.archived ? "Unarchive" : "Archive"), /*#__PURE__*/React.createElement("button", {
    onClick: () => upsert("agreements", {
      ...data.agreements.find(x => x.id === a.id),
      hidden: !a.hidden
    }),
    className: "text-slate-600 text-xs px-2"
  }, a.hidden ? "Unhide" : "Hide"), /*#__PURE__*/React.createElement("button", {
    onClick: () => ask("Delete this agreement and its receipts? Linked disbursements remain.", () => remove("agreements", a.id)),
    className: "text-rose-600 text-xs px-2"
  }, "Delete"))))))), tab === "disbursements" && /*#__PURE__*/React.createElement(DisbTab, {
    fDisb: fDisb,
    data: data,
    agNum: agNum,
    collapsedAg: collapsedAg,
    setCollapsedAg: setCollapsedAg,
    expanded: expanded,
    setExpanded: setExpanded,
    setModal: setModal,
    upsert: upsert,
    remove: remove,
    updateDisbPayment: updateDisbPayment,
    ask: ask,
    Badge: Badge
  }), tab === "parties" && /*#__PURE__*/React.createElement(PartiesTab, {
    allPartyNames: allPartyNames,
    partyPick: partyPick,
    setPartyPick: setPartyPick,
    selectedParty: selectedParty,
    setSelectedParty: setSelectedParty,
    partyView: partyView,
    setPartyView: setPartyView,
    partyBalances: partyBalances,
    disbComputed: disbComputed,
    trComputed: trComputed,
    data: data,
    setModal: setModal,
    upsert: upsert,
    Badge: Badge
  }), tab === "transfers" && /*#__PURE__*/React.createElement(TransfersTab, {
    fTr: fTr,
    setModal: setModal,
    remove: remove,
    ask: ask
  ...
  }, /*#__PURE__*/React.createElement(PartyPanel, {
    data: data,
    save: save,
    addParty: addParty,
    ask: ask,
    ...
    ask: ask
  }), /*#__PURE__*/React.createElement(CurrencyPanel, {
    data: data,
    addCurrency: addCurrency,
    save: save,
    ask: ask
  }), isSuperAdmin && /*#__PURE__*/React.createElement(ReleasePanel, {
    release: release,
    readOnly: readOnly,
    currentHost: pagePrint(),
    legacyHostOnly: legacyHostOnly,
    publishRelease: publishRelease,
    setEntryLock: setEntryLock,
    ask: ask
  }), /*#__PURE__*/React.createElement(UserPanel, {
    users: users,
    maxUsers: MAX_USERS,
  ...
  }, "Register Seed"), /*#__PURE__*/React.createElement("p", {
    className: "text-xs text-slate-500 mb-3"
  }, "Load all agreements, received invoices, and per-party allocations. This replaces all current data."), /*#__PURE__*/React.createElement("button", {
    onClick: loadRegister,
    className: "bg-blue-700 hover:bg-blue-600 text-white text-sm px-4 py-2 rounded-lg shadow-sm"
  }, "Load / Reload Register Data")), /*#__PURE__*/React.createElement(ImportPanel, {
    data: data,
    save: save,
    ask: ask
    ...
    onSave: (a, allocs) => {
      saveAgreementWithAllocs(a, allocs);
      setModal(null);
    }
  }), modal?.type === "invoice" && /*#__PURE__*/React.createElement(InvoiceForm, {
    agreement: modal.payload.agreement,
    initial: modal.payload.invoice,
    currencies: data.currencies,
    suggestedNumber: nextInvNumber(data.agreements, modal.payload.agreement),
    suggestedFrom: numberBasis(data.agreements, modal.payload.agreement),
    onClose: () => setModal(null),
    onSave: (iv, addToValue) => {
      saveInvoice(modal.payload.agreement.id, iv, addToValue);
      setModal(null);
    },
    onDelete: () => {
      const iv = modal.payload.invoice;
      const agId = modal.payload.agreement.id;
      setModal(null);
      ask(`Delete invoice ${iv.number || ""}? The agreement value is not adjusted back automatically.`, () => deleteInvoice(agId, iv.id));
    }
  }), modal?.type === "receipt" && /*#__PURE__*/React.createElement(MoneyForm, {
    title: modal.payload.receipt ? "Edit Payment" : `Payment Received — ${modal.payload.agreement.title}`,
    currency: modal.payload.agreement.currency,
    currencies: data.currencies,
    initial: modal.payload.receipt,
    verb: "Receipt",
    invoices: modal.payload.agreement.invoices || [],
    receipts: modal.payload.agreement.receipts || [],
    summaryFor: ({
      amount,
      invoiceId,
      id
      ...
      const rows = [{
        ...
        label: "Invoiced",
        value: `${csym(cur)} ${fmt(invoicedTot)}`
      }, out < -0.005 ? {
        label: "Received beyond invoiced",
        value: `${csym(cur)} ${fmt(Math.abs(out))}`,
        bold: true,
        danger: true
      } : {
        label: "Outstanding after",
        value: `${csym(cur)} ${fmt(out)}`,
        bold: true
      }];
      ...
      if (iv) {
        ...
        rows.push({
          label: `Balance on ${iv.number || "invoice"}`,
          value: `${csym(cur)} ${fmt(bal)}`,
          bold: true,
          danger: bal < -0.005
        });
      }
    ...
    onDelete: () => {
      ...
      ask(`Delete this payment of ${csym(modal.payload.agreement.currency)} ${fmt(r.amount)} received on ${r.date}? Any allocations recorded against it are removed too.`, () => {
        ...
        if (ag) upsert("agreements", {
          ...ag,
          receipts: (ag.receipts || []).filter(q => q.id !== r.id)
        });
      });
    },
    onClose: () => setModal(null),
    onSave: r => {
      if (blockLocked(modal.payload.agreement.id)) {
        setModal(null);
        return;
      }
      const ag = data.agreements.find(x => x.id === modal.payload.agreement.id);
      const receipts = (ag.receipts || []).some(q => q.id === r.id) ? ag.receipts.map(q => q.id === r.id ? r : q) : [...(ag.receipts || []), r];
      upsert("agreements", {
        ...ag,
        receipts
      });
  ...
  }), modal?.type === "disbursement" && /*#__PURE__*/React.createElement(DisbursementForm, {
    initial: modal.payload.edit,
    presetAgreement: modal.payload.presetAgreement,
    agreements: data.agreements,
    ...
    onClose: () => setModal(null),
    onSave: d => {
      if (blockLocked(d.agreementId)) {
        setModal(null);
        return;
      }
      upsert("disbursements", d);
      setModal(null);
    }
    ...
    verb: "Payment",
    summaryTitle: "Effect on Disbursement",
    summaryFor: ({
      amount,
      id
    }) => {
    ...
    onDelete: () => {
      ...
      ask(`Delete this payment of ${csym(pay.currency || modal.payload.disb.currency)} ${fmt(pay.amount)} to ${modal.payload.disb.party}?`, () => {
        ...
        if (dd) upsert("disbursements", {
          ...dd,
          payments: (dd.payments || []).filter(q => q.id !== pay.id)
        });
      });
    },
    onClose: () => setModal(null),
    onSave: p => {
      if (blockLocked(modal.payload.disb.agreementId)) {
        setModal(null);
        return;
      }
  ...
  }), modal?.type === "cellDisb" && /*#__PURE__*/React.createElement(CellDisbForm, {
    agreement: modal.payload.agreement,
    receipt: modal.payload.receipt,
    party: modal.payload.party,
    initial: modal.payload.disbId ? data.disbursements.find(x => x.id === modal.payload.disbId) : null,
    currencies: data.currencies,
    allDisb: data.disbursements,
    onClose: () => setModal(null),
    onSave: dd => {
      if (blockLocked(modal.payload.agreement.id)) {
        setModal(null);
      ...
      upsert("disbursements", dd);
      setModal(null);
    },
    onDelete: () => {
      const id = modal.payload.disbId;
      const party = modal.payload.party;
      setModal(null);
      ask(`Delete the disbursement to ${party} on this agreement? Payments recorded against it are removed too.`, () => remove("disbursements", id));
    }
  }), modal?.type === "transfer" && /*#__PURE__*/React.createElement(TransferForm, {
    initial: modal.payload.edit,
    presetFrom: modal.payload.presetFrom,
    parties: data.parties,
  ...
  }), reportModal && /*#__PURE__*/React.createElement(ReportModal, {
    allParties: allPartyNames,
    activeParties: Object.keys(partyBalances),
    agreements: data.agreements,
    onClose: () => setReportModal(false),
    onPreview: (list, agIds) => {
      setPreviewHtml(generatePrettyReport(list, true, agIds));
    },
    onGenerate: (which, list, agIds) => {
      if (which === "pretty" || which === "both") generatePrettyReport(list, false, agIds);
      if (which === "excel" || which === "both") exportExcel(list, agIds);
    ...
    className: "bg-white px-4 py-2.5 flex items-center justify-between shadow",
    ...
    onClick: () => openOrSave(previewHtml, `Funds_Flow_Report_${new Date().toISOString().slice(0, 10)}.html`),
    className: "text-xs px-3 py-1.5 rounded-lg bg-slate-900 text-white"
  }, "Open in new tab / Print"), /*#__PURE__*/React.createElement("button", {
    onClick: () => {
      if (saveFile(previewHtml, `Funds_Flow_Report_${new Date().toISOString().slice(0, 10)}.html`)) setNotice("Report downloaded.");
    },
    ...
    className: "no-print fixed bottom-5 right-5 z-50 max-w-sm bg-slate-900 text-white text-xs px-4 py-3 rounded-lg shadow-2xl flex items-start gap-3"
  }, /*#__PURE__*/React.createElement("span", {
...
function DisbTab({
  ...
  return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    ...
    className: "font-serif text-2xl text-slate-900 tracking-tight"
  }, "Disbursements by Agreement"), /*#__PURE__*/React.createElement("p", {
    className: "text-[10px] uppercase tracking-[0.18em] text-slate-400 mt-1"
  }, fDisb.length, " disbursements")), /*#__PURE__*/React.createElement("div", {
    className: "flex items-center gap-2"
  }, /*#__PURE__*/React.createElement("button", {
    onClick: () => setCollapsedAg(Object.fromEntries([...new Set(fDisb.map(d => d.agreementId || "none"))].map(k => [k, true]))),
    className: "text-[10px] uppercase tracking-[0.14em] px-3 py-2 rounded-lg border border-slate-200 text-slate-600 bg-white hover:border-slate-400 transition-colors"
  }, "Collapse all"), /*#__PURE__*/React.createElement("button", {
    onClick: () => setCollapsedAg({}),
    className: "text-[10px] uppercase tracking-[0.14em] px-3 py-2 rounded-lg border border-slate-200 text-slate-600 bg-white hover:border-slate-400 transition-colors"
  }, "Expand all"), /*#__PURE__*/React.createElement("button", {
    onClick: () => setModal({
      type: "disbursement",
    ...
    className: "bg-slate-900 hover:bg-slate-700 text-white text-[11px] uppercase tracking-[0.14em] px-4 py-2 rounded-lg shadow-sm transition-colors"
  }, "+ New Disbursement"))), fDisb.length === 0 && /*#__PURE__*/React.createElement("div", {
    className: "bg-white rounded-xl border border-slate-200 p-8 text-center text-slate-400 shadow-sm"
  }, "No disbursements match the current filters."), (() => {
    const groups = {};
    fDisb.forEach(d => {
      const k = d.agreementId || "none";
    ...
    return keys.map(k => {
      ...
      return /*#__PURE__*/React.createElement("div", {
        ...
        onClick: () => setCollapsedAg(c => ({
          ...c,
          [k]: !c[k]
        })),
        className: `bg-gradient-to-r from-slate-900 to-slate-800 text-white px-5 py-3.5 ${collapsedAg[k] ? "rounded-xl" : "rounded-t-xl"} flex flex-wrap items-center gap-3 cursor-pointer hover:from-slate-800 hover:to-slate-700 transition-all`
      }, /*#__PURE__*/React.createElement("span", {
        className: "text-slate-400 text-xs w-3 shrink-0"
      }, collapsedAg[k] ? "▸" : "▾"), /*#__PURE__*/React.createElement("span", {
        className: "text-slate-400 tabular-nums text-sm w-6 shrink-0"
      }, ag ? String(agNum[ag.id]).padStart(2, "0") : "—"), /*#__PURE__*/React.createElement("span", {
        className: "text-base tracking-wide w-44 truncate shrink-0"
      }, ag ? ag.title : "Unlinked"), /*#__PURE__*/React.createElement("span", {
        className: "text-[10px] uppercase tracking-[0.16em] text-slate-400 w-20 shrink-0"
      }, list.length, " ", list.length === 1 ? "party" : "parties"), /*#__PURE__*/React.createElement("div", {
        className: "ml-auto flex items-center gap-8 text-xs text-slate-300"
      }, /*#__PURE__*/React.createElement("span", {
        className: "flex items-baseline gap-2"
      }, /*#__PURE__*/React.createElement("span", {
        className: "text-[10px] uppercase tracking-[0.14em] text-slate-500"
      }, "Allocated"), /*#__PURE__*/React.createElement("b", {
        className: "text-white tabular-nums inline-block text-right",
        style: {
          minWidth: "7.5rem"
        }
      }, "$ ", fmt(allocTot2))), /*#__PURE__*/React.createElement("span", {
        className: "flex items-baseline gap-2"
      }, /*#__PURE__*/React.createElement("span", {
        className: "text-[10px] uppercase tracking-[0.14em] text-slate-500"
      }, "Paid"), /*#__PURE__*/React.createElement("b", {
        className: "text-white tabular-nums inline-block text-right",
        style: {
          minWidth: "7.5rem"
        }
      }, "$ ", fmt(paidTot))), agLock ? /*#__PURE__*/React.createElement("span", {
        className: "flex items-center gap-1.5 text-[10px] uppercase tracking-[0.14em] text-slate-400 whitespace-nowrap"
      }, /*#__PURE__*/React.createElement("svg", {
        width: "9",
        height: "9",
        viewBox: "0 0 24 24",
        ...
        onClick: e => {
          ...
          setModal({
            type: "disbursement",
            payload: {
              presetAgreement: k === "none" ? "" : k
            }
          });
        },
        title: "Add another party to this contract",
        className: "border border-slate-600 hover:border-slate-300 hover:bg-slate-700 text-slate-200 text-[10px] uppercase tracking-[0.14em] px-2.5 py-1.5 rounded-md transition-colors whitespace-nowrap"
      }, "+ Party"))), !collapsedAg[k] && /*#__PURE__*/React.createElement("div", {
        className: "border border-t-0 border-slate-200 rounded-b-xl divide-y divide-slate-100 bg-white"
      }, list.map(d => /*#__PURE__*/React.createElement("div", {
        key: d.id
      }, /*#__PURE__*/React.createElement("div", {
        className: "px-4 py-2.5 flex flex-wrap items-center gap-3 cursor-pointer hover:bg-slate-50 transition-colors",
        ...
        s: d.paymentStatus
      }), d.feePercent ? /*#__PURE__*/React.createElement("span", {
        className: "text-[11px] text-slate-400 px-1.5 py-0.5 rounded bg-slate-100"
      }, "fee ", d.feePercent, "%") : null, /*#__PURE__*/React.createElement("div", {
        ...
        className: "text-slate-400 font-normal text-xs mr-1"
      }, d.currency), fmt(d.amount)), /*#__PURE__*/React.createElement("div", {
        ...
        className: "text-left px-3 py-1.5"
      }, "Comment"), /*#__PURE__*/React.createElement("th", null))), /*#__PURE__*/React.createElement("tbody", null, (d.payments || []).length === 0 && /*#__PURE__*/React.createElement("tr", null, /*#__PURE__*/React.createElement("td", {
        colSpan: 7,
        className: "px-3 py-3 text-center text-slate-400 text-xs"
      }, "No payments made yet.")), (d.payments || []).map(p => /*#__PURE__*/React.createElement("tr", {
        key: p.id,
        className: "border-t border-slate-100"
      }, /*#__PURE__*/React.createElement("td", {
        className: "px-3 py-1.5 whitespace-nowrap"
        ...
        onBlur: e => updateDisbPayment(d.id, p.id, {
      ...
      }, "$ ", fmt(p.amount * p.rate)), /*#__PURE__*/React.createElement("td", {
        className: "px-3 py-1.5"
      }, /*#__PURE__*/React.createElement("input", {
        ...
        readOnly: agLock,
        placeholder: agLock ? "" : "…",
        className: `w-32 rounded px-1 py-0.5 text-xs border ${agLock ? "border-transparent bg-transparent text-slate-500 cursor-not-allowed" : "border-transparent hover:border-slate-200 focus:border-slate-300"}`
      })), /*#__PURE__*/React.createElement("td", {
        ...
        className: "text-blue-700 text-xs mr-2"
      }, "Edit"), /*#__PURE__*/React.createElement("button", {
        onClick: () => upsert("disbursements", {
          ...data.disbursements.find(x => x.id === d.id),
          payments: d.payments.filter(q => q.id !== p.id)
        }),
        ...
        className: "bg-blue-600 hover:bg-blue-500 text-white text-xs px-3 py-1.5 rounded-lg shadow-sm"
      }, "+ Onward Transfer from ", d.party)) : /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("button", {
        onClick: () => setModal({
          type: "disbPayment",
          payload: {
            disb: d,
        ...
        className: "bg-rose-600 hover:bg-rose-500 text-white text-xs px-3 py-1.5 rounded-lg shadow-sm"
      }, "+ Record Payment to ", d.party), /*#__PURE__*/React.createElement("button", {
        onClick: () => setModal({
          type: "transfer",
          payload: {
            presetFrom: d.party
        ...
        className: "bg-blue-600 hover:bg-blue-500 text-white text-xs px-3 py-1.5 rounded-lg shadow-sm"
      }, "+ Onward Transfer from ", d.party), /*#__PURE__*/React.createElement("button", {
        onClick: () => setModal({
          type: "disbursement",
          payload: {
            edit: d
        ...
        onClick: () => ask("Delete this disbursement and its payments?", () => remove("disbursements", d.id)),
        className: "text-rose-600 text-xs px-2"
      }, "Delete"))))))));
    });
  })());
...
function PartiesTab({
  ...
  const partyRows = allPartyNames.filter(p => partyPick.length === 0 || partyPick.includes(p)).map(p => {
    ...
    return {
      ...
      ...b,
      holding: b.inUSD - b.outUSD,
      disbCount: disbComputed.filter(d => d.party === p).length,
      trCount: trComputed.filter(t => t.fromParty === p).length
    };
  });
  if (selectedParty) {
    const b = partyBalances[selectedParty] || {
      inUSD: 0,
      outUSD: 0
    };
    const stRows = [...disbComputed.filter(d => d.party === selectedParty).flatMap(d => (d.payments || []).map(p => ({
      date: p.date,
      kind: "in",
      desc: `Received — ${d.agreementTitle}${d.description ? ` · ${d.description}` : ""}`,
      currency: p.currency || d.currency,
      ...
      usd: Number(p.amount) * Number(p.rate),
      comment: p.notes || ""
    }))), ...trComputed.filter(t => t.fromParty === selectedParty).map(t => ({
      date: t.date,
      kind: "out",
      desc: `Transfer out — ${t.accountName} (${t.accountCurrency}) · ${t.payType}`,
      currency: t.currency,
    ...
    const feeRows = disbComputed.filter(d => d.party === selectedParty).map(d => {
      ...
      const feePct = Number(d.feePercent || 0);
      const fee = received * feePct / 100;
      return {
        id: d.id,
        ...
        fee,
        paid,
        net: received - fee - paid,
        comment: d.comment,
        payments: d.payments || [],
        raw: d,
        locked: isLocked(data.agreements.find(x => x.id === d.agreementId))
      };
    });
    const feeTot = feeRows.reduce((a, r) => ({
    ...
    return /*#__PURE__*/React.createElement("div", {
    ...
    }, "(NOT Service Fee)"))), /*#__PURE__*/React.createElement("div", {
      className: "flex gap-1 border-b border-slate-200"
    }, [["receipts", "Agreement Receipts"], ["remittance", `Onwards Remittances (From ${selectedParty})`]].map(([k, l]) => /*#__PURE__*/React.createElement("button", {
      key: k,
      onClick: () => setPartyView(k),
      className: `px-4 py-2 text-sm font-medium border-b-2 transition-colors ${partyView === k ? "border-slate-900 text-slate-900" : "border-transparent text-slate-500 hover:text-slate-800"}`
      ...
      l: "Remitted Onward (USD)",
      v: b.outUSD,
      c: "text-blue-700",
      bar: "bg-blue-500"
    }].map(x => /*#__PURE__*/React.createElement("div", {
      key: x.l,
      className: "relative bg-white rounded-lg shadow-sm border border-slate-200 px-3 py-2.5 overflow-hidden"
    }, /*#__PURE__*/React.createElement("span", {
      className: `absolute top-0 left-0 right-0 h-0.5 ${x.bar}`
    }), /*#__PURE__*/React.createElement("p", {
      className: "text-[9px] uppercase tracking-[0.16em] text-slate-400 font-medium truncate"
    }, x.l), /*#__PURE__*/React.createElement("p", {
      className: `text-base font-medium mt-0.5 tabular-nums ${Number(x.v) < -0.005 ? "text-rose-600" : x.c}`
    }, "$ ", fmt(x.v))))), partyView === "receipts" && /*#__PURE__*/React.createElement("div", {
      className: "bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden"
    }, /*#__PURE__*/React.createElement("div", {
      className: "px-5 py-2 border-b border-slate-200 font-semibold text-sm"
    }, "Agreement Receipts & Service Fees"), /*#__PURE__*/React.createElement("div", {
      className: "overflow-x-auto"
    }, /*#__PURE__*/React.createElement("table", {
      className: "w-full text-sm min-w-[900px]"
    }, /*#__PURE__*/React.createElement("thead", {
      className: "bg-slate-800 text-white text-xs uppercase"
    }, /*#__PURE__*/React.createElement("tr", null, /*#__PURE__*/React.createElement("th", {
    ...
    }, "Amount Received (USD)"), /*#__PURE__*/React.createElement("th", {
      className: "text-right px-4 py-2.5"
    }, "Fee %"), /*#__PURE__*/React.createElement("th", {
      className: "text-right px-4 py-2.5"
    }, "Service Fee (USD)"), /*#__PURE__*/React.createElement("th", {
      className: "text-right px-4 py-2.5"
    }, "Paid (USD)"), /*#__PURE__*/React.createElement("th", {
      className: "text-right px-4 py-2.5"
    }, "Total Due (USD)"), /*#__PURE__*/React.createElement("th", {
      className: "text-left px-4 py-2.5"
    }, "Comments"), /*#__PURE__*/React.createElement("th", {
      className: "px-4 py-2.5"
    }))), /*#__PURE__*/React.createElement("tbody", null, feeRows.length === 0 && /*#__PURE__*/React.createElement("tr", null, /*#__PURE__*/React.createElement("td", {
      colSpan: 9,
      className: "px-4 py-6 text-center text-slate-400"
    }, "No agreement allocations to this party.")), feeRows.map(r => /*#__PURE__*/React.createElement(Fragment, {
      key: r.id
    }, /*#__PURE__*/React.createElement("tr", {
      className: "border-t border-slate-200 bg-slate-50"
    }, /*#__PURE__*/React.createElement("td", {
      className: "px-4 py-2 font-medium"
    }, r.agreement), /*#__PURE__*/React.createElement("td", {
      className: "px-4 py-2"
    }, /*#__PURE__*/React.createElement(Badge, {
      s: r.status
    })), /*#__PURE__*/React.createElement("td", {
      className: "px-4 py-2 text-right"
    }, fmt(r.received)), /*#__PURE__*/React.createElement("td", {
      className: "px-4 py-2 text-right"
    }, r.locked ? /*#__PURE__*/React.createElement("span", {
      className: "text-slate-500 text-xs tabular-nums"
    }, r.feePct) : /*#__PURE__*/React.createElement("input", {
      type: "number",
      step: "0.1",
      defaultValue: r.feePct,
      onBlur: e => upsert("disbursements", {
        ...r.raw,
    ...
    }, r.fee ? `(${fmt(r.fee)})` : fmt(0)), /*#__PURE__*/React.createElement("td", {
      className: `px-4 py-2 text-right tabular-nums ${r.paid > 0.005 ? "text-emerald-700" : "text-slate-300"}`
    }, fmt(r.paid)), /*#__PURE__*/React.createElement("td", {
      className: `px-4 py-2 text-right font-semibold tabular-nums ${r.net > 0.005 ? "text-slate-900" : r.net < -0.005 ? "text-rose-600" : "text-slate-300"}`
    }, fmt(r.net)), /*#__PURE__*/React.createElement("td", {
      className: "px-4 py-2 text-xs text-slate-500"
    }, r.comment), /*#__PURE__*/React.createElement("td", {
      ...
      className: "text-emerald-700 text-xs mr-2"
    }, "+ Txn"), /*#__PURE__*/React.createElement("button", {
      onClick: () => setModal({
        type: "disbursement",
        payload: {
          edit: r.raw
    ...
    }, "Edit")))), r.payments.map(p => /*#__PURE__*/React.createElement("tr", {
      key: p.id,
    ...
    }, fmt(Number(p.amount) * Number(p.rate))), /*#__PURE__*/React.createElement("td", null), /*#__PURE__*/React.createElement("td", {
      className: "px-4 py-1.5 text-slate-400"
    }, p.notes), /*#__PURE__*/React.createElement("td", {
    ...
    }, "Edit")))))), feeRows.length > 0 && /*#__PURE__*/React.createElement("tr", {
      className: "bg-blue-50 font-bold border-t-2 border-slate-400"
    }, /*#__PURE__*/React.createElement("td", {
      className: "px-4 py-2.5",
      colSpan: 2
    }, "TOTAL"), /*#__PURE__*/React.createElement("td", {
      className: "px-4 py-2.5 text-right"
    }, fmt(feeTot.received)), /*#__PURE__*/React.createElement("td", null), /*#__PURE__*/React.createElement("td", {
      className: "px-4 py-2.5 text-right text-rose-600"
    }, "(", fmt(feeTot.fee), ")"), /*#__PURE__*/React.createElement("td", {
      className: "px-4 py-2.5 text-right text-emerald-700 tabular-nums"
    }, fmt(feeTot.paid)), /*#__PURE__*/React.createElement("td", {
      className: `px-4 py-2.5 text-right tabular-nums ${feeTot.net > 0.005 ? "text-slate-900" : feeTot.net < -0.005 ? "text-rose-600" : "text-slate-400"}`
    }, fmt(feeTot.net)), /*#__PURE__*/React.createElement("td", {
      colSpan: 2
    })))))), partyView === "remittance" && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("div", {
      className: "flex justify-end"
    }, /*#__PURE__*/React.createElement("button", {
      onClick: () => setModal({
        type: "transfer",
        payload: {
          presetFrom: selectedParty
        }
      }),
      className: "bg-blue-600 hover:bg-blue-500 text-white text-sm px-4 py-2 rounded-lg shadow-sm"
    }, "+ Onwards Remittance from ", selectedParty)), /*#__PURE__*/React.createElement("div", {
      className: "bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden"
    }, /*#__PURE__*/React.createElement("div", {
      className: "px-5 py-2 border-b border-slate-200 font-semibold text-sm"
    }, "Associated Accounts"), /*#__PURE__*/React.createElement("div", {
      className: "overflow-x-auto"
    }, /*#__PURE__*/React.createElement("table", {
    ...
    }, "Transfers"), /*#__PURE__*/React.createElement("th", {
      className: "text-right px-5 py-2"
    }, "Total Sent (USD)"), /*#__PURE__*/React.createElement("th", {
      className: "text-left px-5 py-2"
    }, "Last Transfer"))), /*#__PURE__*/React.createElement("tbody", null, assocAccounts.length === 0 && /*#__PURE__*/React.createElement("tr", null, /*#__PURE__*/React.createElement("td", {
      colSpan: 5,
      className: "px-5 py-5 text-center text-slate-400"
    }, "No transfers to accounts yet from this party.")), assocAccounts.map(a => /*#__PURE__*/React.createElement("tr", {
      key: a.id,
      className: "border-t border-slate-100"
    }, /*#__PURE__*/React.createElement("td", {
      className: "px-5 py-2 font-medium"
      ...
      className: "px-5 py-2"
    }, a.currency), /*#__PURE__*/React.createElement("td", {
      className: "px-5 py-2 text-right"
    }, a.count), /*#__PURE__*/React.createElement("td", {
      className: "px-5 py-2 text-right"
    }, fmt(a.usd)), /*#__PURE__*/React.createElement("td", {
      className: "px-5 py-2"
    }, a.last))))))), /*#__PURE__*/React.createElement("div", {
      className: "bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden"
    }, /*#__PURE__*/React.createElement("div", {
      className: "px-5 py-2 border-b border-slate-200 font-semibold text-sm"
    }, "Full Statement (USD)"), /*#__PURE__*/React.createElement("div", {
      className: "overflow-x-auto"
    }, /*#__PURE__*/React.createElement("table", {
      ...
      className: "text-right px-5 py-2"
    }, "In (USD)"), /*#__PURE__*/React.createElement("th", {
      className: "text-right px-5 py-2"
    }, "Out (USD)"), /*#__PURE__*/React.createElement("th", {
      className: "text-right px-5 py-2"
    }, "Balance (USD)"), /*#__PURE__*/React.createElement("th", {
      className: "text-left px-5 py-2"
    }, "Comment"))), /*#__PURE__*/React.createElement("tbody", null, stWithBal.length === 0 && /*#__PURE__*/React.createElement("tr", null, /*#__PURE__*/React.createElement("td", {
      colSpan: 6,
      className: "px-5 py-6 text-center text-slate-400"
    }, "No movements yet for this party.")), stWithBal.map((r, i) => /*#__PURE__*/React.createElement("tr", {
      key: i,
    ...
    }, "(", r.currency, " ", fmt(r.amount), ")")), /*#__PURE__*/React.createElement("td", {
      className: "px-5 py-2 text-right text-emerald-700"
    }, r.kind === "in" ? "$ " + fmt(r.usd) : ""), /*#__PURE__*/React.createElement("td", {
      className: "px-5 py-2 text-right text-blue-700"
    }, r.kind === "out" ? "$ " + fmt(r.usd) : ""), /*#__PURE__*/React.createElement("td", {
      className: "px-5 py-2 text-right font-medium"
    }, "$ ", fmt(r.balance)), /*#__PURE__*/React.createElement("td", {
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "font-serif text-lg mr-2"
  }, "Parties (", partyRows.length, ")"), /*#__PURE__*/React.createElement("span", {
    className: "text-xs text-slate-500"
  }, "Filter:"), allPartyNames.map(p => /*#__PURE__*/React.createElement("button", {
    key: p,
    onClick: () => setPartyPick(partyPick.includes(p) ? partyPick.filter(x => x !== p) : [...partyPick, p]),
    className: `px-3 py-1 rounded-full text-xs border transition-colors ${partyPick.includes(p) ? "bg-slate-900 text-white border-slate-900" : "bg-white text-slate-600 border-slate-300 hover:border-slate-500"}`
  }, p)), partyPick.length > 0 && /*#__PURE__*/React.createElement("button", {
    onClick: () => setPartyPick([]),
  ...
  }, "Disbursements"), /*#__PURE__*/React.createElement("th", {
    className: "text-right px-5 py-2.5"
  }, "Received (USD)"), /*#__PURE__*/React.createElement("th", {
    className: "text-right px-5 py-2.5"
  }, "Paid Onward (USD)"), /*#__PURE__*/React.createElement("th", {
    className: "text-right px-5 py-2.5"
  }, "Holding (USD)"), /*#__PURE__*/React.createElement("th", {
    className: "text-right px-5 py-2.5"
  }, "Transfers"), /*#__PURE__*/React.createElement("th", null))), /*#__PURE__*/React.createElement("tbody", null, partyRows.length === 0 && /*#__PURE__*/React.createElement("tr", null, /*#__PURE__*/React.createElement("td", {
    colSpan: 7,
    className: "px-5 py-6 text-center text-slate-400"
  }, "No parties yet.")), partyRows.map(p => /*#__PURE__*/React.createElement("tr", {
    key: p.name,
    className: "border-t border-slate-100 hover:bg-slate-50 cursor-pointer transition-colors",
    onClick: () => setSelectedParty(p.name)
  }, /*#__PURE__*/React.createElement("td", {
    className: "px-5 py-2.5 font-medium text-blue-800"
  }, /*#__PURE__*/React.createElement(PartyName, {
    name: p.name
  })), /*#__PURE__*/React.createElement("td", {
    className: "px-5 py-2.5 text-right"
  }, p.disbCount), /*#__PURE__*/React.createElement("td", {
    className: "px-5 py-2.5 text-right text-emerald-700"
  }, fmt(p.inUSD)), /*#__PURE__*/React.createElement("td", {
    className: "px-5 py-2.5 text-right text-blue-700"
  }, fmt(p.outUSD)), /*#__PURE__*/React.createElement("td", {
    className: "px-5 py-2.5 text-right font-medium"
  }, fmt(p.holding)), /*#__PURE__*/React.createElement("td", {
    className: "px-5 py-2.5 text-right"
...
function TransfersTab({
  ...
  return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    ...
    className: "bg-slate-50 text-slate-500 text-xs uppercase"
  }, /*#__PURE__*/React.createElement("tr", null, ["Date", "From Party", "To Account", "Partial/Full", "Currency", "Amount", "Rate → USD", "USD Equivalent", "Comment", ""].map(h => /*#__PURE__*/React.createElement("th", {
    key: h,
    className: "text-left px-4 py-2.5"
  }, h)))), /*#__PURE__*/React.createElement("tbody", null, fTr.length === 0 && /*#__PURE__*/React.createElement("tr", null, /*#__PURE__*/React.createElement("td", {
    colSpan: 10,
    className: "px-4 py-6 text-center text-slate-400"
  }, "No transfers match the current filters.")), fTr.map(t => /*#__PURE__*/React.createElement("tr", {
    key: t.id,
    ...
    className: "px-4 py-2"
  }, t.accountName, " ", /*#__PURE__*/React.createElement("span", {
    className: "text-xs text-slate-400"
  }, "(", t.accountCurrency, ")")), /*#__PURE__*/React.createElement("td", {
    className: "px-4 py-2"
  }, /*#__PURE__*/React.createElement("span", {
    className: `px-2 py-0.5 rounded-full text-xs font-medium ring-1 ring-inset ${t.payType === "Full" ? "bg-emerald-50 text-emerald-700 ring-emerald-600/20" : "bg-amber-50 text-amber-700 ring-amber-600/20"}`
  }, t.payType)), /*#__PURE__*/React.createElement("td", {
    className: "px-4 py-2"
  }, t.currency), /*#__PURE__*/React.createElement("td", {
    className: "px-4 py-2 text-right"
  }, csym(t.currency), " ", fmt(t.amount)), /*#__PURE__*/React.createElement("td", {
    className: "px-4 py-2 text-right"
  }, Number(t.rate).toFixed(4)), /*#__PURE__*/React.createElement("td", {
    className: "px-4 py-2 text-right font-medium"
  }, "$ ", fmt(t.usd)), /*#__PURE__*/React.createElement("td", {
    className: "px-4 py-2 text-slate-500 text-xs"
  }, t.notes), /*#__PURE__*/React.createElement("td", {
    ...
    onClick: () => ask("Delete this transfer?", () => remove("transfers", t.id)),
    className: "text-rose-600 text-xs"
  }, "Delete"))))))));
}
function AuthScreen({
  ...
  useEffect(() => {
    if (hasUsers) setMode(m => m === "firstSetup" ? "signin" : m);
  }, [hasUsers]);
  ...
  const submit = async () => {
    ...
    try {
      if (mode === "signin") {
        if (!userId || !pw) {
          setErr("Enter your user ID and password.");
          return;
        }
        ...
        if (!userId || !answer) {
          setErr("Enter your user ID and answer.");
          return;
        }
        if (newPw.length < 6) {
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "bg-gradient-to-r from-slate-950 to-slate-900 text-white p-6 flex items-center gap-3"
  }, /*#__PURE__*/React.createElement("div", {
    className: "w-10 h-10 rounded-md border border-slate-600 flex items-center justify-center font-serif text-lg tracking-widest"
  }, "X"), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("h1", {
    ...
    onKeyDown: e => e.key === "Enter" && submit(),
    className: "w-full border border-slate-300 rounded-lg px-3 py-2 text-sm"
  })), needsCreds && /*#__PURE__*/React.createElement(React.Fragment, null, /*#__PURE__*/React.createElement("label", {
    className: "block mb-3"
  }, /*#__PURE__*/React.createElement("span", {
    className: "block text-xs font-medium text-slate-600 mb-1"
    ...
    className: "block text-xs font-medium text-slate-600 mb-1"
  }, "Security Question"), /*#__PURE__*/React.createElement("select", {
    value: qChoice,
    onChange: e => {
      setQChoice(e.target.value);
      if (e.target.value !== "Custom…") setQuestion(e.target.value);else setQuestion("");
    },
    ...
    key: q
  }, q))), qChoice === "Custom…" && /*#__PURE__*/React.createElement("input", {
    value: question,
  ...
  }, "Forgot password?")), mode === "firstSetup" && /*#__PURE__*/React.createElement("div", {
    className: "mt-4 text-xs text-center"
  }, /*#__PURE__*/React.createElement("button", {
    onClick: () => goto("signin"),
    className: "text-blue-700 hover:underline"
  }, "Already have a login? Sign in")), !hasUsers && mode === "signin" && /*#__PURE__*/React.createElement("div", {
    className: "mt-4 text-xs text-center"
  }, /*#__PURE__*/React.createElement("button", {
    onClick: () => goto("firstSetup"),
    className: "text-slate-500 hover:underline"
  }, "← First-time setup")), /*#__PURE__*/React.createElement("p", {
...
function ReleasePanel({
  ...
  const go = () => {
    ask(includeData ? `Release ${version} and overwrite the stored records with the data in this session? Data entry will then be locked to this page.` : `Release ${version}? Existing records are left untouched, and data entry will be locked to this page.`, async () => {
      setBusy(true);
      try {
        await publishRelease(version, includeData);
      } finally {
        setBusy(false);
      }
    });
  ...
  return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("h2", {
    ...
    className: "grid grid-cols-2 gap-3 text-xs mb-4 pb-4 border-b border-slate-100"
  }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("span", {
    className: "block text-[10px] uppercase tracking-[0.14em] text-slate-400 mb-0.5"
  }, "Current release"), /*#__PURE__*/React.createElement("span", {
    className: "text-slate-800 font-medium"
  }, release?.version || "Not released"), /*#__PURE__*/React.createElement("span", {
    className: "block text-[10px] text-slate-400"
  }, "this build: ", APP_VERSION)), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("span", {
    className: "block text-[10px] uppercase tracking-[0.14em] text-slate-400 mb-0.5"
  }, "Released"), /*#__PURE__*/React.createElement("span", {
    className: "text-slate-800"
  }, stamp(release?.publishedAt))), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("span", {
    className: "block text-[10px] uppercase tracking-[0.14em] text-slate-400 mb-0.5"
  }, "By"), /*#__PURE__*/React.createElement("span", {
    className: "text-slate-800"
  }, release?.publishedBy || "—")), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("span", {
    className: "block text-[10px] uppercase tracking-[0.14em] text-slate-400 mb-0.5"
  }, "Code build"), /*#__PURE__*/React.createElement("span", {
    className: "text-slate-800 tabular-nums"
  }, BUILD)), /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("span", {
    className: "block text-[10px] uppercase tracking-[0.14em] text-slate-400 mb-0.5"
  }, "Data entry"), release?.locked ? /*#__PURE__*/React.createElement("span", {
    className: `inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md text-[11px] font-medium ring-1 ring-inset ${readOnly ? "bg-amber-50 text-amber-700 ring-amber-600/20" : "bg-emerald-50 text-emerald-700 ring-emerald-600/20"}`
  }, /*#__PURE__*/React.createElement("span", {
    className: `w-1.5 h-1.5 rounded-full ${readOnly ? "bg-amber-500" : "bg-emerald-500"}`
  }), readOnly ? "Locked elsewhere" : "Open on this page") : /*#__PURE__*/React.createElement("span", {
    className: "inline-flex items-center gap-1.5 px-2 py-0.5 rounded-md text-[11px] font-medium ring-1 ring-inset bg-slate-100 text-slate-600 ring-slate-500/20"
  }, /*#__PURE__*/React.createElement("span", {
    className: "w-1.5 h-1.5 rounded-full bg-slate-400"
  }), "Open everywhere"))), legacyHostOnly && /*#__PURE__*/React.createElement("p", {
    ...
    className: "block text-xs text-slate-500 mb-1"
  }, "This page"), /*#__PURE__*/React.createElement("div", {
    className: "border border-slate-200 rounded-lg px-3 py-2 text-xs bg-slate-50 text-slate-500 truncate",
    title: currentHost
  }, currentHost))), /*#__PURE__*/React.createElement("label", {
  ...
  }, "Also push the data currently loaded in this session ", /*#__PURE__*/React.createElement("span", {
    className: "block text-[11px] text-slate-400"
  }, "Leave unticked to publish structure only and keep every existing record exactly as it is. Tick it to overwrite stored records with what you see now."))), /*#__PURE__*/React.createElement("button", {
    disabled: busy,
    onClick: go,
    className: "w-full bg-rose-600 hover:bg-rose-500 disabled:opacity-50 text-white text-sm font-medium py-2.5 rounded-lg shadow-sm tracking-wide"
  }, busy ? "…" : `PUBLISH ${version}`)), /*#__PURE__*/React.createElement("div", {
    className: "bg-white rounded-xl shadow-sm border border-slate-200 p-4"
  }, /*#__PURE__*/React.createElement("h3", {
    className: "text-sm font-semibold mb-1"
  }, "Data entry lock"), /*#__PURE__*/React.createElement("p", {
    className: "text-xs text-slate-500 mb-3"
  }, release?.locked ? "Only the page recorded at release time can write. Other copies are read-only." : "Any copy of the app can currently write to the shared records."), /*#__PURE__*/React.createElement("div", {
    className: "flex flex-wrap gap-2"
  }, /*#__PURE__*/React.createElement("button", {
    onClick: () => setEntryLock(true),
    className: "bg-slate-900 hover:bg-slate-700 text-white text-xs px-3 py-2 rounded-lg shadow-sm"
  }, "Lock entry to this page"), /*#__PURE__*/React.createElement("button", {
    onClick: () => ask("Unlock data entry? Any copy of the app will be able to edit records again.", () => setEntryLock(false)),
    disabled: !release?.locked,
    className: "border border-slate-300 disabled:opacity-40 text-slate-600 text-xs px-3 py-2 rounded-lg"
  }, "Unlock everywhere")), /*#__PURE__*/React.createElement("p", {
    className: "text-[10px] text-slate-400 mt-3"
  }, "If you ever lock yourself out, sign in as Super Admin on the page that still has access and unlock from here.")));
}
function UserPanel({
  users,
  maxUsers,
  ...
  return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("h2", {
    ...
    className: "normal-case text-[10px] tracking-normal text-slate-400"
  }, "(fixed)")), /*#__PURE__*/React.createElement("th", {
    className: "text-left px-4 py-2"
  }, "Type"), /*#__PURE__*/React.createElement("th", {
    className: "text-left px-4 py-2"
  }, "Status"), /*#__PURE__*/React.createElement("th", null))), /*#__PURE__*/React.createElement("tbody", null, users.length === 0 && /*#__PURE__*/React.createElement("tr", null, /*#__PURE__*/React.createElement("td", {
    colSpan: 5,
    className: "px-4 py-4 text-center text-slate-400"
  }, "No users.")), users.map(u => /*#__PURE__*/React.createElement("tr", {
    key: u.userId,
    className: "border-t border-slate-100"
  }, /*#__PURE__*/React.createElement("td", {
    className: "px-4 py-2"
  }, editing && editing.userId === u.userId ? /*#__PURE__*/React.createElement("span", {
    ...
    className: "border border-slate-300 rounded px-2 py-1 text-sm w-40 focus:border-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-100"
  }), /*#__PURE__*/React.createElement("button", {
    onClick: saveName,
  ...
  }, u.name || "No name set"), u.userId === currentUser && /*#__PURE__*/React.createElement("span", {
    className: "text-[10px] uppercase tracking-wide text-emerald-700"
  }, "You"))), /*#__PURE__*/React.createElement("td", {
    className: "px-4 py-2"
  }, /*#__PURE__*/React.createElement("span", {
    className: "flex items-center gap-1.5 text-slate-600",
    title: "The user ID / email is the login credential and cannot be changed"
  }, /*#__PURE__*/React.createElement("svg", {
    width: "10",
    height: "10",
    viewBox: "0 0 24 24",
  ...
  }, u.userId))), /*#__PURE__*/React.createElement("td", {
    className: "px-4 py-2"
  }, /*#__PURE__*/React.createElement("span", {
    ...
    className: "text-xs text-slate-500"
  }, "Only the ", /*#__PURE__*/React.createElement("b", null, "Super Admin"), " can create, reset or remove user IDs. You can change your own display name; the user ID / email itself is fixed.")) : /*#__PURE__*/React.createElement("div", {
    className: "bg-white rounded-xl shadow-sm border border-slate-200 p-4"
  }, !showForm ? /*#__PURE__*/React.createElement("button", {
    onClick: () => setShowForm(true),
    disabled: !canAdd,
    className: "bg-slate-900 hover:bg-slate-700 disabled:opacity-40 text-white text-sm px-4 py-2 rounded-lg shadow-sm"
  }, "+ Create User ID") : /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("h3", {
    className: "text-sm font-semibold mb-1"
    ...
    className: "block text-xs text-slate-500 mb-1"
  }, "User ID / email"), /*#__PURE__*/React.createElement("input", {
    autoFocus: true,
    value: userId,
    onChange: e => setUserId(e.target.value),
    className: "w-full border border-slate-300 rounded-lg px-3 py-2 text-sm",
    ...
    disabled: busy,
    onClick: submit,
    className: "flex-1 bg-slate-900 hover:bg-slate-700 disabled:opacity-50 text-white py-2 rounded-lg text-sm font-medium shadow-sm"
...
function Modal({
  ...
  return /*#__PURE__*/React.createElement("div", {
    ...
    className: "bg-white rounded-xl shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto border border-slate-200",
    onClick: e => e.stopPropagation()
  }, /*#__PURE__*/React.createElement("div", {
    className: "px-4 sm:px-6 py-4 border-b border-slate-100 flex justify-between items-center sticky top-0 bg-white z-10"
  }, /*#__PURE__*/React.createElement("h3", {
    className: "font-serif text-base sm:text-lg text-slate-900"
...
const Field = ({
  ...
  className: "block text-[10px] font-semibold text-slate-500 uppercase tracking-[0.14em] mb-1.5"
}, label), children);
...
function ReportModal({
  ...
  return /*#__PURE__*/React.createElement(Modal, {
  ...
  }, [["pretty", "Pretty PDF"], ["excel", "Excel"], ["both", "Both"]].map(([k, l]) => /*#__PURE__*/React.createElement("button", {
    key: k,
    onClick: () => setWhich(k),
    className: `py-2 rounded-lg text-sm border transition-colors ${which === k ? "bg-slate-900 text-white border-slate-900" : "bg-white text-slate-600 border-slate-300 hover:border-slate-500"}`
  }, l)))), /*#__PURE__*/React.createElement(Field, {
    label: "Report scope"
  }, /*#__PURE__*/React.createElement("div", {
    className: "grid grid-cols-2 gap-2"
  }, [["party", "By party"], ["agreement", "By agreement"]].map(([k, l]) => /*#__PURE__*/React.createElement("button", {
    key: k,
    onClick: () => setScope(k),
    className: `py-2 rounded-lg text-sm border transition-colors ${scope === k ? "bg-slate-900 text-white border-slate-900" : "bg-white text-slate-600 border-slate-300 hover:border-slate-500"}`
  }, l)))), scope === "party" ? /*#__PURE__*/React.createElement(Field, {
    label: "Include party statements for"
  }, /*#__PURE__*/React.createElement("div", {
    className: "flex flex-wrap gap-2 mb-2"
  }, /*#__PURE__*/React.createElement("button", {
    onClick: () => setPick(allParties),
    className: "text-xs text-blue-700 underline"
  }, "Select all"), /*#__PURE__*/React.createElement("button", {
    onClick: () => setPick(activeParties),
    className: "text-xs text-blue-700 underline"
  }, "Only active"), /*#__PURE__*/React.createElement("button", {
    onClick: () => setPick([]),
    className: "text-xs text-blue-700 underline"
  }, "None")), /*#__PURE__*/React.createElement("div", {
    className: "flex flex-wrap gap-2 max-h-44 overflow-y-auto border border-slate-200 rounded-lg p-2"
  }, allParties.length === 0 && /*#__PURE__*/React.createElement("span", {
    className: "text-xs text-slate-400"
  }, "No parties yet."), allParties.map(p => /*#__PURE__*/React.createElement("button", {
    key: p,
    onClick: () => toggle(p),
    ...
    onClick: () => setAgPick(agreements.map(a => a.id)),
    className: "text-xs text-blue-700 underline"
  }, "Select all"), /*#__PURE__*/React.createElement("button", {
    onClick: () => setAgPick([]),
    className: "text-xs text-blue-700 underline"
    ...
    className: "flex flex-wrap gap-2 max-h-44 overflow-y-auto border border-slate-200 rounded-lg p-2"
  }, agreements.length === 0 && /*#__PURE__*/React.createElement("span", {
    className: "text-xs text-slate-400"
  }, "No agreements yet."), agreements.map(a => /*#__PURE__*/React.createElement("button", {
    key: a.id,
    onClick: () => toggleAg(a.id),
...
function NotesPanel({
  ...
  return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("div", {
    className: "flex flex-wrap justify-between items-baseline gap-3 mb-4"
  }, /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("h2", {
    className: "font-serif text-2xl text-slate-900 tracking-tight"
  }, "Notes"), /*#__PURE__*/React.createElement("p", {
    className: "text-[10px] uppercase tracking-[0.18em] text-slate-400 mt-1"
  }, filtered ? `${shown.length} of ${notes.length}` : notes.length, " note", notes.length === 1 ? "" : "s")), /*#__PURE__*/React.createElement("div", {
    className: "flex items-center gap-2"
  }, /*#__PURE__*/React.createElement("input", {
    value: q,
    ...
    className: "flex flex-wrap items-end gap-x-4 gap-y-2 mb-5 p-3 bg-slate-50 border border-slate-200 rounded-xl"
  }, /*#__PURE__*/React.createElement("label", {
    className: "flex flex-col gap-1"
  }, /*#__PURE__*/React.createElement("span", {
    className: "text-[10px] uppercase tracking-[0.14em] text-slate-400"
  }, "Agreement"), /*#__PURE__*/React.createElement("select", {
    value: agFilter,
    onChange: e => setAgFilter(e.target.value),
    className: selCls
  }, /*#__PURE__*/React.createElement("option", {
    value: "all"
  }, "All agreements"), hasUnlinked && /*#__PURE__*/React.createElement("option", {
    value: "none"
  }, "No agreement"), agOpts.map(a => /*#__PURE__*/React.createElement("option", {
    key: a.id,
    value: a.id
  }, (a.ref ? a.ref + " · " : "") + a.title)))), /*#__PURE__*/React.createElement("label", {
    className: "flex flex-col gap-1"
  }, /*#__PURE__*/React.createElement("span", {
    className: "text-[10px] uppercase tracking-[0.14em] text-slate-400"
    ...
    value: "all"
  }, "Anyone"), hasUnassigned && /*#__PURE__*/React.createElement("option", {
    value: "none"
  }, "Unassigned"), userOpts.map(u => /*#__PURE__*/React.createElement("option", {
    key: u.userId,
    value: u.userId
  }, u.name ? `${u.name} (${u.userId})` : u.userId)))), /*#__PURE__*/React.createElement("label", {
    className: "flex flex-col gap-1"
  }, /*#__PURE__*/React.createElement("span", {
    className: "text-[10px] uppercase tracking-[0.14em] text-slate-400"
    ...
    onClick: clearAll,
    className: "text-xs text-slate-500 hover:text-slate-800 underline underline-offset-2 pb-2"
  ...
  }, notes.length ? /*#__PURE__*/React.createElement(React.Fragment, null, "No notes match the current filters. ", filtered && /*#__PURE__*/React.createElement("button", {
    onClick: clearAll,
    className: "not-italic text-blue-700 hover:underline"
  }, "Clear filters")) : "No notes yet. Use the + button at the bottom-right of any page, or “New Note”.")), shown.map(n => /*#__PURE__*/React.createElement("tr", {
    key: n.id,
...
function PartySelect({
  ...
  if (adding) return /*#__PURE__*/React.createElement("div", {
    ...
    className: "bg-slate-900 text-white text-xs px-3 rounded-lg whitespace-nowrap"
  }, "Save"), /*#__PURE__*/React.createElement("button", {
    onClick: () => {
      setAdding(false);
      setNewName("");
...
function AgreementForm({
  ...
  const addRow = (name, amount) => {
    ...
    setAllocs(a => ({
      ...a,
      [name]: Number(amount) || 0
    }));
    setPickParty("");
    setPickAmt("");
  };
  const setAmt = (name, v) => setAllocs(a => ({
    ...a,
    [name]: v === "" ? "" : Number(v)
  }));
  const dropRow = name => setAllocs(a => {
    const c = {
      ...a
    };
    delete c[name];
  ...
  return /*#__PURE__*/React.createElement(Modal, {
    ...
    value: f.status,
    onChange: set("status")
  }, AG_STATUSES.map(s => /*#__PURE__*/React.createElement("option", {
    key: s
  }, s)))), /*#__PURE__*/React.createElement(Field, {
    label: "Payment Status"
  }, /*#__PURE__*/React.createElement("select", {
    className: inp,
    value: f.paymentStatus || "Ongoing",
    ...
    name: name,
    className: "font-medium text-slate-700"
  }), locked.has(name) && /*#__PURE__*/React.createElement("svg", {
    width: "10",
    height: "10",
    viewBox: "0 0 24 24",
    fill: "none",
    ...
    step: "0.01",
    value: allocs[name],
    onChange: e => setAmt(name, e.target.value),
    readOnly: agClosed,
    className: `w-full rounded px-2 py-1 text-sm text-right tabular-nums border ${agClosed ? "border-transparent bg-transparent text-slate-500 cursor-not-allowed" : "border-slate-200 focus:border-slate-400 focus:outline-none focus:ring-2 focus:ring-slate-100"}`
  }))), /*#__PURE__*/React.createElement("td", {
    ...
    onClick: () => {
      const n = addParty(newParty, "disbursement");
      if (n) {
        addRow(n, 0);
        setAdding(false);
        setNewParty("");
    ...
    value: pickParty,
    onChange: e => {
      if (e.target.value === "__add__") setAdding(true);else setPickParty(e.target.value);
    },
    ...
    step: "0.01",
    value: pickAmt,
    onChange: e => setPickAmt(e.target.value),
    onKeyDown: e => {
    ...
    disabled: !pickParty,
    onClick: () => addRow(pickParty, pickAmt),
    ...
    className: "px-3 py-2 border-t border-slate-200 flex flex-wrap justify-between gap-x-6 gap-y-1 text-xs bg-white"
  }, /*#__PURE__*/React.createElement("span", {
    ...
    className: "text-slate-800 tabular-nums ml-1"
  }, f.currency, " ", fmt(contract))), /*#__PURE__*/React.createElement("span", {
    className: Math.abs(variance) < 0.005 ? "text-emerald-700" : "text-rose-600"
  }, Math.abs(variance) < 0.005 ? "Fully allocated" : /*#__PURE__*/React.createElement(React.Fragment, null, "Unallocated ", /*#__PURE__*/React.createElement("b", {
    className: "tabular-nums ml-1"
  }, fmt(variance)))))), /*#__PURE__*/React.createElement(Field, {
    label: "Comment"
  }, /*#__PURE__*/React.createElement("textarea", {
    className: inp,
    rows: 2,
  ...
  }, "Parties saved here appear as disbursement lines under this contract, where they can be edited, paid or deleted."));
}
function InvoiceForm({
  ...
  return /*#__PURE__*/React.createElement(Modal, {
    title: initial ? "Edit Invoice" : `Record Invoice — ${agreement.title}`,
    onClose: onClose
  }, /*#__PURE__*/React.createElement("div", {
    className: "grid grid-cols-1 sm:grid-cols-2 gap-x-4"
  }, /*#__PURE__*/React.createElement(Field, {
    ...
    label: `Rate to USD ${rateHint(cur)}`
  }, /*#__PURE__*/React.createElement("input", {
    type: "number",
    step: "0.0001",
    className: inp,
    value: f.rate,
    onChange: set("rate")
  }))), f.amount && f.rate && /*#__PURE__*/React.createElement("p", {
    className: "text-sm text-slate-600 mb-3"
  }, "USD equivalent: ", /*#__PURE__*/React.createElement("span", {
    className: "font-semibold"
  }, fmt(f.amount * f.rate))), /*#__PURE__*/React.createElement("div", {
  ...
  }, "Effect on Agreement"), /*#__PURE__*/React.createElement("div", {
    className: "px-3 py-2 flex justify-between"
  }, /*#__PURE__*/React.createElement("span", {
    className: "text-slate-500"
  }, "Invoiced before"), /*#__PURE__*/React.createElement("span", {
    className: "tabular-nums"
  }, csym(cur), " ", fmt(already))), /*#__PURE__*/React.createElement("div", {
    className: "px-3 py-2 flex justify-between border-t border-slate-100"
  }, /*#__PURE__*/React.createElement("span", {
    className: "text-slate-500"
  }, "Invoiced after"), /*#__PURE__*/React.createElement("b", {
    className: "tabular-nums"
  }, csym(cur), " ", fmt(nextTotal))), /*#__PURE__*/React.createElement("div", {
    className: "px-3 py-2 flex justify-between border-t border-slate-100"
  }, /*#__PURE__*/React.createElement("span", {
    className: "text-slate-500"
  }, "Contract value"), /*#__PURE__*/React.createElement("span", {
    className: "tabular-nums"
  }, csym(cur), " ", fmt(contract))), /*#__PURE__*/React.createElement("div", {
    className: `px-3 py-2 flex justify-between border-t border-slate-100 ${over > 0.005 ? "text-rose-600" : "text-slate-500"}`
  }, /*#__PURE__*/React.createElement("span", null, over > 0.005 ? "Exceeds contract by" : "Remaining to invoice"), /*#__PURE__*/React.createElement("b", {
    className: "tabular-nums"
  }, csym(cur), " ", fmt(Math.abs(over))))), /*#__PURE__*/React.createElement("label", {
    className: "flex items-start gap-2.5 mb-4 cursor-pointer"
  }, /*#__PURE__*/React.createElement("input", {
    type: "checkbox",
    checked: addToValue,
    ...
    disabled: !f.amount || !f.rate,
    onClick: () => onSave({
      ...f,
      amount: Number(f.amount),
      rate: Number(f.rate)
    }, addToValue),
    className: "flex-1 bg-slate-900 hover:bg-slate-700 disabled:opacity-40 text-white py-2 rounded-lg text-sm font-medium shadow-sm"
  }, initial ? "Save Invoice" : "Record Invoice")));
}
function MoneyForm({
  title,
  currency,
  ...
  const rows = summaryFor ? summaryFor({
    amount: Number(f.amount) || 0,
    rate: Number(f.rate) || 0,
    invoiceId: f.invoiceId,
    id: f.id
  }) : null;
  const blockMsg = blockWhen ? blockWhen({
    amount: Number(f.amount) || 0,
    id: f.id
  }) : null;
  return /*#__PURE__*/React.createElement(Modal, {
    title: title,
    onClose: onClose
  }, invList.length > 0 && /*#__PURE__*/React.createElement(Field, {
    label: "Associated Invoice"
  }, /*#__PURE__*/React.createElement("select", {
    className: inp,
    value: f.invoiceId || "",
    onChange: e => {
      const iv = invList.find(x => x.id === e.target.value);
      setF({
        ...f,
        invoiceId: e.target.value,
        amount: f.amount || (iv ? iv.amount : ""),
        rate: f.rate || (iv ? iv.rate : f.rate)
      });
    }
  }, /*#__PURE__*/React.createElement("option", {
    value: ""
    ...
    label: `Exchange Rate to USD ${rateHint(currency)}`
  }, /*#__PURE__*/React.createElement("input", {
    type: "number",
    step: "0.0001",
    className: inp,
    ...
    className: "font-semibold"
  }, fmt(f.amount * f.rate))), rows && rows.length > 0 && /*#__PURE__*/React.createElement("div", {
    className: "border border-slate-200 rounded-lg mb-4 text-xs"
  }, /*#__PURE__*/React.createElement("div", {
    className: "px-3 py-2 bg-slate-50 border-b border-slate-200 text-[10px] font-semibold text-slate-500 uppercase tracking-[0.14em]"
  }, summaryTitle || "Effect on Agreement"), rows.map((row, i) => /*#__PURE__*/React.createElement("div", {
    key: row.label,
    className: `px-3 py-2 flex justify-between ${i ? "border-t border-slate-100" : ""} ${row.danger ? "text-rose-600" : ""}`
  }, /*#__PURE__*/React.createElement("span", {
    className: row.danger ? "" : "text-slate-500"
  }, row.label), row.bold ? /*#__PURE__*/React.createElement("b", {
    className: "tabular-nums"
  }, row.value) : /*#__PURE__*/React.createElement("span", {
    className: "tabular-nums"
  }, row.value)))), /*#__PURE__*/React.createElement(Field, {
    label: "Comment"
  }, /*#__PURE__*/React.createElement("input", {
    className: inp,
    value: f.notes,
    ...
    disabled: !f.amount || !f.rate || !!blockMsg,
    onClick: () => onSave({
      ...f,
      amount: Number(f.amount),
      rate: Number(f.rate)
    }),
    className: "flex-1 bg-emerald-600 hover:bg-emerald-500 disabled:opacity-40 text-white py-2 rounded-lg text-sm font-medium shadow-sm"
  }, "Save ", verb)));
}
function DisbursementForm({
  ...
  return /*#__PURE__*/React.createElement(Modal, {
    title: initial ? "Edit Disbursement" : "New Disbursement",
    onClose: onClose
  }, /*#__PURE__*/React.createElement(Field, {
    label: "Source Agreement (any, including past agreements)"
  }, /*#__PURE__*/React.createElement("select", {
    className: inp,
    value: f.agreementId || "",
    onChange: e => setF({
    ...
    label: "Disburse to Party"
  }, agSelected ? allocatedParties.length ? /*#__PURE__*/React.createElement("select", {
    className: inp,
    value: f.party,
    onChange: e => setF({
      ...f,
    ...
    parties: parties,
    addParty: addParty
  }), agSelected && allocatedParties.length > 0 && /*#__PURE__*/React.createElement("p", {
    className: "mt-1 text-[10px] text-slate-400"
  }, "Only parties allocated on this agreement appear here. To add one, edit the agreement's allocations.")), /*#__PURE__*/React.createElement(Field, {
    label: "Description"
  }, /*#__PURE__*/React.createElement("input", {
    className: inp,
    value: f.description,
    ...
    label: `Allocated Amount (${f.currency})`
  }, /*#__PURE__*/React.createElement("input", {
    type: "number",
    className: inp,
    value: f.amount,
    ...
    value: f.comment || "",
    onChange: set("comment")
  })), amountBelowPaid && /*#__PURE__*/React.createElement("p", {
    className: "mb-3 px-3 py-2 bg-rose-50 border border-rose-200 rounded-lg text-[11px] text-rose-700"
  }, "Allocated amount is below the ", csym(f.currency), " ", fmt(paidOnRecord), " already disbursed on this record. The allocation can't be less than what has been paid."), /*#__PURE__*/React.createElement("button", {
    disabled: !f.party || !f.amount || !partyAllowed || amountBelowPaid,
    onClick: () => onSave({
      ...f,
      amount: Number(f.amount),
      feePercent: f.feePercent === "" ? 0 : Number(f.feePercent)
    }),
    className: "w-full bg-slate-900 hover:bg-slate-700 disabled:opacity-40 text-white py-2 rounded-lg text-sm font-medium shadow-sm"
  }, "Save Disbursement"));
}
function CellDisbForm({
  agreement,
  receipt,
  party,
  initial,
  ...
  return /*#__PURE__*/React.createElement(Modal, {
    title: initial ? `Edit Disbursement — ${party}` : `Disburse to ${party}`,
    onClose: onClose
  }, /*#__PURE__*/React.createElement("p", {
...
function TransferForm({
  ...
  return /*#__PURE__*/React.createElement(Modal, {
    title: initial ? "Edit Transfer" : "New Onward Transfer",
    onClose: onClose
  }, /*#__PURE__*/React.createElement(Field, {
    label: "From Party (entity holding funds)"
  }, /*#__PURE__*/React.createElement(PartySelect, {
    value: f.fromParty,
    onChange: v => setF({
      ...f,
      fromParty: v
    ...
    label: "To Account"
  }, addingAcc ? /*#__PURE__*/React.createElement("div", {
    className: "flex gap-2 flex-wrap"
  }, /*#__PURE__*/React.createElement("input", {
    autoFocus: true,
    className: "border border-slate-300 rounded-lg px-3 py-2 text-sm flex-1 min-w-[140px]",
    value: accName,
    ...
    onClick: () => {
      const id = addAccount(accName, accCur, "", f.fromParty);
      if (id) {
        pickAccount(id);
        setAddingAcc(false);
        setAccName("");
    ...
    className: "bg-slate-900 text-white text-xs px-3 rounded-lg"
  }, "Save"), /*#__PURE__*/React.createElement("button", {
    ...
    value: f.accountId || "",
    onChange: e => {
      if (e.target.value === "__add__") setAddingAcc(true);else pickAccount(e.target.value);
    }
  }, /*#__PURE__*/React.createElement("option", {
    value: ""
    ...
    key: a.id,
    value: a.id
  }, a.name, " (", a.currency, ")"))), /*#__PURE__*/React.createElement("optgroup", {
    label: "Other accounts"
  }, accounts.filter(a => !f.fromParty || a.party !== f.fromParty).sort((a, b) => orderName(a.name, b.name)).map(a => /*#__PURE__*/React.createElement("option", {
    key: a.id,
    value: a.id
  ...
  }, /*#__PURE__*/React.createElement("option", null, "Full"), /*#__PURE__*/React.createElement("option", null, "Partial"))), /*#__PURE__*/React.createElement(Field, {
    label: "Currency"
  }, /*#__PURE__*/React.createElement("select", {
    className: inp,
    value: f.currency,
    ...
    label: `Rate to USD ${rateHint(f.currency)}`
  }, /*#__PURE__*/React.createElement("input", {
    type: "number",
    step: "0.0001",
    className: inp,
    ...
    className: "font-semibold"
  }, fmt(f.amount * f.rate))), /*#__PURE__*/React.createElement(Field, {
    label: "Comment"
  }, /*#__PURE__*/React.createElement("input", {
    className: inp,
    value: f.notes,
    ...
    className: "w-full bg-blue-600 hover:bg-blue-500 disabled:opacity-40 text-white py-2 rounded-lg text-sm font-medium shadow-sm"
  }, "Save Transfer"));
}
function PartyPanel({
  data,
  ...
  const renameParty = (oldName, newName) => {
    ...
    save({
      ...data,
      parties: data.parties.map(p => p.name === oldName ? {
        ...p,
        name: newName
      } : p),
      disbursements: data.disbursements.map(d => d.party === oldName ? {
        ...d,
        party: newName
      } : d),
      transfers: data.transfers.map(t => t.fromParty === oldName ? {
        ...t,
        fromParty: newName
      } : t),
      accounts: data.accounts.map(a => a.party === oldName ? {
        ...a,
        party: newName
      } : a)
    });
  ...
  return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("h2", {
    className: "font-serif text-lg mb-4"
  }, "Parties / Entities"), /*#__PURE__*/React.createElement("div", {
    className: "bg-white rounded-xl shadow-sm border border-slate-200 mb-4 overflow-hidden"
  }, /*#__PURE__*/React.createElement("table", {
    className: "w-full text-sm"
  }, /*#__PURE__*/React.createElement("thead", {
    className: "bg-slate-50 text-xs uppercase text-slate-500"
    ...
    className: "text-left px-4 py-2"
  }, "Used For"), /*#__PURE__*/React.createElement("th", {
    ...
    return /*#__PURE__*/React.createElement("tr", {
      ...
      className: "px-4 py-2 font-medium"
    }, editing && editing.id === p.id ? /*#__PURE__*/React.createElement("span", {
      className: "flex gap-1 items-center"
    }, /*#__PURE__*/React.createElement("input", {
      autoFocus: true,
      value: editing.name,
      ...
      onKeyDown: e => {
        if (e.key === "Enter") renameParty(p.name, editing.name);
        if (e.key === "Escape") setEditing(null);
      },
      className: "border border-slate-300 rounded px-2 py-1 text-sm w-32"
    }), /*#__PURE__*/React.createElement("button", {
      onClick: () => renameParty(p.name, editing.name),
      className: "text-emerald-700 text-xs"
    }, "Save"), /*#__PURE__*/React.createElement("button", {
      onClick: () => setEditing(null),
      className: "text-slate-400 text-xs"
      ...
      onChange: e => save({
        ...data,
        parties: data.parties.map(x => x.id === p.id ? {
          ...x,
          type: e.target.value
        } : x)
      }),
      ...
      value: "disbursement"
    }, "Disbursements"), /*#__PURE__*/React.createElement("option", {
      value: "receivable"
    }, "Clients (income)"), /*#__PURE__*/React.createElement("option", {
      value: "both"
    }, "Both"))), /*#__PURE__*/React.createElement("td", {
      ...
      className: "text-blue-700 text-xs mr-3"
    }, "Edit"), used === 0 ? /*#__PURE__*/React.createElement("button", {
      onClick: () => ask(`Remove "${p.name}"?`, () => save({
        ...data,
        parties: data.parties.filter(x => x.id !== p.id)
      })),
      className: "text-rose-600 text-xs"
    ...
    className: "block text-xs text-slate-500 mb-1"
  }, "Used for"), /*#__PURE__*/React.createElement("select", {
...
function AccountPanel({
  ...
  const renameAccount = (id, newName) => {
    ...
    save({
      ...data,
      accounts: data.accounts.map(a => a.id === id ? {
        ...a,
        name: newName
      } : a)
    });
  ...
  return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("h2", {
    ...
    className: "px-4 py-2 font-medium"
  }, editing && editing.id === a.id ? /*#__PURE__*/React.createElement("span", {
    className: "flex gap-1 items-center"
  }, /*#__PURE__*/React.createElement("input", {
    autoFocus: true,
    value: editing.name,
    ...
    onKeyDown: e => {
      if (e.key === "Enter") renameAccount(a.id, editing.name);
      if (e.key === "Escape") setEditing(null);
    },
    className: "border border-slate-300 rounded px-2 py-1 text-sm w-40"
  }), /*#__PURE__*/React.createElement("button", {
    onClick: () => renameAccount(a.id, editing.name),
    className: "text-emerald-700 text-xs"
  }, "Save"), /*#__PURE__*/React.createElement("button", {
    onClick: () => setEditing(null),
    className: "text-slate-400 text-xs"
    ...
    onChange: e => save({
      ...data,
      accounts: data.accounts.map(x => x.id === a.id ? {
        ...x,
        party: e.target.value
      } : x)
    }),
    ...
    className: "px-4 py-2 text-xs text-slate-500"
  }, a.comment), /*#__PURE__*/React.createElement("td", {
    className: "px-4 py-2 text-right whitespace-nowrap"
  }, /*#__PURE__*/React.createElement("button", {
    onClick: () => setEditing({
      id: a.id,
    ...
    onClick: () => ask(`Remove account "${a.name}"? Existing transfers keep their records.`, () => save({
      ...data,
      accounts: data.accounts.filter(x => x.id !== a.id)
    })),
    className: "text-rose-600 text-xs"
    ...
    value: name,
    onChange: e => setName(e.target.value),
    placeholder: "e.g. ENBD USD Main"
  })), /*#__PURE__*/React.createElement("label", {
    className: "block"
  }, /*#__PURE__*/React.createElement("span", {
    className: "block text-xs text-slate-500 mb-1"
  }, "Party"), /*#__PURE__*/React.createElement("select", {
    className: "border border-slate-300 rounded-lg px-3 py-2 text-sm",
    value: party,
  ...
  }, data.currencies.map(c => /*#__PURE__*/React.createElement("option", {
    key: c.code
  }, c.code)))), /*#__PURE__*/React.createElement("label", {
    className: "block flex-1 min-w-[140px]"
  }, /*#__PURE__*/React.createElement("span", {
    className: "block text-xs text-slate-500 mb-1"
    ...
    onClick: () => {
      addAccount(name, cur, comment, party);
      setName("");
      setComment("");
      setParty("");
    },
...
function CurrencyPanel({
  ...
  return /*#__PURE__*/React.createElement("div", null, /*#__PURE__*/React.createElement("h2", {
    className: "font-serif text-lg mb-4"
  }, "Currencies (base: USD)"), /*#__PURE__*/React.createElement("div", {
    className: "bg-white rounded-xl shadow-sm border border-slate-200 mb-4 overflow-hidden"
  }, /*#__PURE__*/React.createElement("table", {
    className: "w-full text-sm"
  }, /*#__PURE__*/React.createElement("thead", {
    ...
    className: "text-left px-4 py-2"
  }, "Rate to USD"), /*#__PURE__*/React.createElement("th", {
    className: "text-left px-4 py-2"
  }, "Mode"), /*#__PURE__*/React.createElement("th", null))), /*#__PURE__*/React.createElement("tbody", null, data.currencies.map(c => /*#__PURE__*/React.createElement("tr", {
    key: c.code,
    className: "border-t border-slate-100"
  }, /*#__PURE__*/React.createElement("td", {
    className: "px-4 py-2 font-medium"
    ...
    className: "px-4 py-2"
  }, c.code === "USD" ? "1.00" : c.fixed && c.rate ? Number(c.rate).toFixed(4) : "Entered per transaction"), /*#__PURE__*/React.createElement("td", {
    className: "px-4 py-2 text-xs text-slate-500"
  }, c.code === "USD" ? "Base currency" : c.fixed ? "Fixed default" : "Daily rate (manual)"), /*#__PURE__*/React.createElement("td", {
    className: "px-4 py-2 text-right"
  }, !["USD", "AED"].includes(c.code) && /*#__PURE__*/React.createElement("button", {
    onClick: () => ask(`Remove ${c.code}? Existing records keep it.`, () => save({
      ...data,
      currencies: data.currencies.filter(x => x.code !== c.code)
    })),
    className: "text-rose-600 text-xs"
  }, "Remove"))))))), /*#__PURE__*/React.createElement("div", {
    className: "bg-white rounded-xl shadow-sm border border-slate-200 p-4"
    ...
    className: "text-sm font-semibold mb-3"
  }, "Add Currency"), /*#__PURE__*/React.createElement("div", {
    className: "flex flex-wrap gap-2 items-end"
  }, /*#__PURE__*/React.createElement("label", {
    className: "block"
  }, /*#__PURE__*/React.createElement("span", {
    ...
    className: "block text-xs text-slate-500 mb-1"
  }, "Fixed rate to USD (optional)"), /*#__PURE__*/React.createElement("input", {
    type: "number",
    step: "0.0001",
    ...
    className: "text-xs text-slate-400 mt-2"
  }, "Currencies without a fixed rate prompt you for the day's rate to USD on each transaction.")));
}
function ImportPanel({
  ...
  const buildFromWorkbook = wb => {
    ...
    const agRows = sheet("Agreements"),
      rcRows = sheet("Receipts"),
      dbRows = sheet("Disbursements"),
      pyRows = sheet("Disb Payments"),
      trRows = sheet("Transfers");
    ...
    agRows.forEach(r => {
      ...
      agreements.push({
        ...
        ref,
        title,
        party: String(r.Client || "").trim(),
        date: xdate(r.Date),
        currency: String(r.Currency || "USD").trim() || "USD",
        totalValue: Number(r["Total Value"]) || 0,
        status: String(r["Agreement Status"] || "Ongoing").trim() || "Ongoing",
        paymentStatus: String(r["Payment Status"] || "Ongoing").trim() || "Ongoing",
        comment: r.Comment || "",
        receipts: [],
        invoices: []
      });
    ...
    sheet("Invoices").forEach(r => {
      const ag = agreements.find(a => a.id === findAg(r.Ref, r.Agreement));
      if (!ag) return;
      ag.invoices.push({
        id: uid(),
        date: xdate(r["Invoice Date"]),
        number: String(r["Invoice #"] || ""),
        dueDate: xdate(r["Due Date"]),
        amount: Number(r.Amount) || 0,
        rate: Number(r["Rate to USD"]) || 1,
        notes: r.Comment || ""
      });
    });
    rcRows.forEach(r => {
    ...
    dbRows.forEach(r => {
      const {
        ref,
        title
      } = parseSrc(r["Source Agreement"]);
      const agId = findAg(ref, title);
      ...
      const d = {
        ...
        date: xdate(r.Date),
        currency: String(r.Currency || "USD").trim() || "USD",
        amount: Number(r["Allocated Amount"]) || 0,
        paymentStatus: String(r["Payment Status"] || "Ongoing").trim() || "Ongoing",
        feePercent: 0,
        comment: r.Comment || "",
        payments: []
    ...
    const accounts = [],
      accKey = {};
    const getAcc = (name, cur) => {
      name = String(name || "").trim();
      if (!name) return "";
      const k = name.toLowerCase();
      if (accKey[k]) return accKey[k];
      const id = uid();
      accounts.push({
        id,
        name,
        currency: String(cur || "USD").trim() || "USD",
        comment: "",
        party: ""
      });
      accKey[k] = id;
    ...
    trRows.forEach(r => {
      transfers.push({
        id: uid(),
        fromParty: String(r["From Party"] || "").trim(),
        accountId: getAcc(r["To Account"], r["Account Currency"]),
        date: xdate(r.Date),
        currency: String(r["Transfer Currency"] || "USD").trim() || "USD",
        amount: Number(r.Amount) || 0,
        rate: Number(r["Rate to USD"]) || 1,
        payType: String(r["Partial/Full"] || "Full").trim() || "Full",
        notes: r.Comment || ""
      });
    });
    ...
    const currencies = DEFAULT_CURRENCIES.concat([...seen].filter(c => !DEFAULT_CURRENCIES.some(x => x.code === c)).map(c => ({
      code: c,
      rate: null,
      fixed: false
    })));
  ...
  const onExcel = async e => {
    ...
    try {
      ...
      ask(`Import will REPLACE all current data with the uploaded file (${summary}). Continue?`, () => {
        save(next);
        setMsg(`Imported ${summary}.`);
      });
    } catch (e2) {
      setErr(e2.message || "Could not read the workbook.");
    }
    setBusy(false);
  };
  const onImages = async () => {
    if (!imgParty) {
      setErr("Pick a party first.");
      return;
    }
    if (!imgFiles.length) {
      setErr("Choose one or more statement images.");
      return;
    }
    setErr("");
    setMsg("");
    ...
    try {
      const content = [];
      for (const f of imgFiles) content.push({
        type: "image",
        source: {
          type: "base64",
      ...
      if (!parties.some(p => p.name === imgParty)) parties.push({
        id: uid(),
        name: imgParty,
        type: "disbursement"
      });
      ...
      if (!disb) {
        disb = {
          ...
          paymentStatus: "Paid",
          feePercent: 0,
          comment: "Imported from statement image",
          payments: []
        };
      ...
      rows.forEach(r => {
        ...
        if (r.direction === "out") {
          transfers.push({
            ...
            rate,
            payType: "Full",
            notes: r.description || "Statement import"
          });
          outN++;
        } else {
          disb.payments = [...disb.payments, {
            id: uid(),
            date: xdate(r.date),
            amount: amt,
            rate,
      ...
      setErr("Could not read the statement image(s). " + (e2.message || ""));
    }
    setBusy(false);
  };
  return /*#__PURE__*/React.createElement("div", {
  ...
  }, /*#__PURE__*/React.createElement("span", null, busy ? "Working…" : "Choose Excel file"), /*#__PURE__*/React.createElement("input", {
    type: "file",
    accept: ".xlsx,.xls",
    disabled: busy,
    onChange: onExcel,
    ...
    value: imgParty,
    onChange: e => setImgParty(e.target.value),
    className: "border border-slate-300 rounded-lg px-3 py-2 text-sm min-w-[160px]"
  }, partyOpts.length === 0 && /*#__PURE__*/React.createElement("option", {
    value: ""
  }, "No parties"), partyOpts.map(p => /*#__PURE__*/React.createElement("option", {
    key: p,
    value: p
  }, p)))), /*#__PURE__*/React.createElement("label", {
    className: "block flex-1 min-w-[180px]"
    ...
    className: "block text-xs text-slate-500 mb-1"
  }, "Statement image(s)"), /*#__PURE__*/React.createElement("input", {
    type: "file",
    accept: "image/*",
    multiple: true,
    ...
    disabled: busy,
    onClick: onImages,
    className: "bg-emerald-600 hover:bg-emerald-500 disabled:opacity-40 text-white text-sm px-4 py-2 rounded-lg shadow-sm"
  }, busy ? "Reading…" : "Read & Import")), imgFiles.length > 0 && /*#__PURE__*/React.createElement("p", {
    className: "text-[11px] text-slate-400 mt-2"
  }, imgFiles.length, " image", imgFiles.length === 1 ? "" : "s", " selected.")), err && /*#__PURE__*/React.createElement("p", {
    className: "text-xs text-rose-600 mt-3"
  }, err), msg && /*#__PURE__*/React.createElement("p", {
